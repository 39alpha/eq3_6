...
!     This INCLUDE file is referenced by EQ6, EQ3NR, XCON3, and XCON6.
!     The relevant arrays are declared in the EQLIB INCLUDE file
!     eqlo8.h.
!     Nxmod alter/supress option strings:
data ukxm(-1) / 'Suppress        '/
data ukxm(0)  / 'Replace         '/
data ukxm(1)  / 'AugmentLogK     '/
data ukxm(2)  / 'AugmentG        '/

! Exchange model option strings for generic ion exchangers:
...
! Units option strings for thermodynamic parameters of generic ion
!     exchangers:
...
data uoptox(1,3) /'Search for phase boundaries and constrain the step size to match'/
data ioptox(1,3) /0/
data uoptox(2,3) /"Search for phase boundaries and print their locations"/
data ioptox(2,3) /1/
...
! iopt(6):
data uopttx(6) /'Clear the ES Solids at the Initial Value of Reaction Progress'/
...
data ioptox(1,10) /0/
data uoptox(2,10) /'Do it, unless numerical problems cause early termination'/
data ioptox(2,10) /1/
data uoptox(3,10) /' '/
data ioptox(3,10) /0/
data uoptox(4,10) /' '/
...
! iopt(12):
data uopttx(12) /'Auto Basis Switching after Newton-Raphson Iteration'/
...
data ioptox(2,18) /0/
data uoptox(3,18) /'Write a TAB file, prepending TABX file data from a previous run'/
data ioptox(3,18) /1/
data uoptox(4,18) /' '/
data ioptox(4,18) /0/
data uoptox(5,18) /' '/
...
data ioptox(1,19) /0/
data uoptox(2,19) /'Write an EQ6 INPUT file with Quartz dissolving, relative rate law'/
data ioptox(2,19) /1/
data uoptox(3,19) /'Write an EQ6 INPUT file with Albite dissolving, TST rate law'/
data ioptox(3,19) /2/
data uoptox(4,19) /'Write an EQ6 INPUT file with Fluid 1 set up for fluid mixing'/
data ioptox(4,19) /3/
data uoptox(5,19) /' '/
data ioptox(5,19) /0/
...
data ioptox(1,20) /0/
data uoptox(2,20) /'Write an EQ6 INPUT file with Fluid 1 set up for fluid mixing'/
data ioptox(2,20) /1/
data uoptox(3,20) /' '/
data ioptox(3,20) /0/
data uoptox(4,20) /' '/
...
! Iopg activity coefficient option strings.
!   iopg(1):
...
! iopg(2):
data uopgtx(2) /'Choice of pH Scale (Rescales Activity Coefficients)'/
data uopgcx(2) /'EQ3NR, EQ6'/

data uopgox(1,2) /'"Internal" pH scale (no rescaling)'/
data iopgox(1,2) /-1/
data uopgox(2,2) /'NBS pH scale (uses the Bates-Guggenheim equation)'/
data iopgox(2,2) /0/
...
data ioprox(3,2) /2/
data uoprox(4,2) /'Print the reactions, log K values, and associated data'/
data ioprox(4,2) /3/
data uoprox(5,2) /' '/
data ioprox(5,2) /0/
...
! iopr(4):
data uoprtx(4) /'Print a Table of Aqueous Species Concentrations, Activities, etc.'/
...
data ioprox(2,5) /1/
data uoprox(3,5) /'Print cation/H+ and anion/H+ activity ratios'/
data ioprox(3,5) /2/
data uoprox(4,5) /'Print ion/H+ activity ratios and neutral species activities'/
data ioprox(4,5) /3/
data uoprox(5,5) /' '/
data ioprox(5,5) /0/

! iopr(6):
data uoprtx(6) /'Print a Table of Aqueous Mass Balance Percentages'/
data uoprcx(6) /'EQ3NR, EQ6'/

data uoprox(1,6) /"Don't print"/
data ioprox(1,6) /-1/
data uoprox(2,6) /'Print those species comprising at least 99% of each mass balance'/
data ioprox(2,6) /0/
...
! iopr(7):
data uoprtx(7) /'Print Tables of Saturation Indices and Affinities'/
data uoprcx(7) /'EQ3NR, EQ6'/

data uoprox(1,7) /"Don't print"/
data ioprox(1,7) /-1/
data uoprox(2,7) /'Print, omitting those phases undersaturated by more than 10 kcal'/
data ioprox(2,7) /0/
...
! iopr(9):
data uoprtx(9) /'Print a Table of Mean Molal Activity Coefficients'/
...
! iopr(10):
data uoprtx(10) /'Print a Tabulation of the Pitzer Interaction Coefficients'/
...
data iodbox(2,1) /1/
data uodbox(3,1) /'Print Level 1 and Level 2 diagnostic messages'/
data iodbox(3,1) /2/
data uodbox(4,1) /' '/
data iodbox(4,1) /0/
data uodbox(5,1) /' '/
...
data iodbox(2,2) /1/
data uodbox(3,2) /'Print Level 1 and Level 2 kinetics diagnostic messages'/
data iodbox(3,2) /2/
data uodbox(4,2) /' '/
data iodbox(4,2) /0/
data uodbox(5,2) /' '/
...
! iodb(3):
data uodbtx(3) /'Print Pre-Newton-Raphson Optimization Information'/
...
! iodb(6):
data uodbtx(6) /'Print Details of Hypothetical Affinity Calculations'/
...
! iodb(7):
data uodbtx(7) /'Print General Search (e.g., for a phase boundary) Information'/
...
data iodbox(2,8) /1/
data uodbox(3,8) /'Print detailed information (including the betar and delvcr vectors)'/
data iodbox(3,8) /2/
data uodbox(4,8) /' '/
data iodbox(4,8) /0/
data uodbox(5,8) /' '/
