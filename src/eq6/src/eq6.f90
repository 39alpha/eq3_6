...
1210 format(/' * Error - (EQ6/eq6) Have iopt(18)= 1, but there',/7x,'is no existing tabx file to which to append.')
    ...
    ! this code.
    call intbs6(jflag, jflgi, narn1a, narn2a, nbaspd, nbtd, nbti, nbtmax, ndrsrd, ndecsp, noutpt, nsta, nstmax, nttyo, uspeca, ubmtbi)
...
1310 format(/' * Note - (EQ6/eq6) The pressure is jumping from',/7x,'the previous run:',/,/'     Initial pressure      = ',1pg12.5,' bars',/'     Previous run pressure = ',1pg12.5,' bars')
    ...
    if (iopt(15) .ge. 1) then
        ...
        call suprdx(jflag, jsflag, narn1a, narn2a, ndrsd, ndrsmx, ndrsrd, nrdxsp, nsta, nstmax)
    end if
    ...
    if (.not.qredox .and. iopt(15).le.0) then
        ...
        !   nst for nsta
        !   uspec for uspeca
        call suprdx(jflag, jsflag, narn1, narn2, ndrs, ndrsmx, ndrsr, nrdxsp, nst, nstmax)
    end if
