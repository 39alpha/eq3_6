subroutine ckphmx(delxi,dlxmin,dph0,dxh1mx,dxval0,eps100,iodb,nodbmx,nord,noutpt,nrd1mx,nttyo,ph0,ph1,phmax,prcinf,qdump,tolxsu,xi0,xi1,xval0)
    ...
    data usearch /'pH reaches the requested maximum value          '/
    data unam24  /'                       '/
...
100 continue
    ...
    if (icount .gt. 10) then
        if (iodb(1).gt.0 .or. iodb(5).gt.0) then
            write (noutpt,1000)
1000 format(/3x,'A scan for where the pH reaches the requested',' maximum value',/3x,'has failed. Dropping to the minimum',' step size.')
        end if
    ...
    if (delxi .gt. dlxmin) then
        if (ph0 .ge. (1. - tolxsu)*phmax) then
            ...
            if (iodb(1).gt.0 .or. iodb(5).gt.0) then
                write (noutpt,1010) ph0,phmax,xi0,delxi
1010 format(/3x,'The base point pH of ',1pe11.4,' is already',' very close',/3x,'to the requested value of ',e11.4,/3x,'at Xi= ',e11.4,'. Have set delxi',' equal to the minimum',/7x,'value of ',e11.4,'.')
    ...
    if (iodb(1).gt.0 .or. iodb(5).gt.0) then
        if (abs(ph1 - phmax) .le. tolsx) then
            write (noutpt,1020) phmax,xi1,delxi
1020 format(/3x,"Taylor's series predict that the pH is",' at the requested',/3x,'maximum value of ',1pe11.4,' at Xi= ',e11.4,',',/3x,'delxi= ',e11.4,'.')
        end if
    end if
