...
1150 format(/' * Warning - (EQ6/eq6) The fugacity of ',a,' is',/7x,1pg12.5,' bars at the start of the run. It is supposed',/7x,'to be fixed at ',e12.5,' bars. There is an insufficient',/7x,'mass of the gas component present in the system to',/7x,'"saturate" it at the desired fugacity. You may wish to',/7x,'restart this run, adding such mass using the moffg',/7x,'parameter on the input file. Add only about 0.5 to 1.0',/7x,'mole at a time.')
    ...
    if (nrct .gt. 0) then
        ...
        ! rate laws.
        call rtcalc(act, afrc1, cdac, csigma, fkrc, idirec, imchmx, imech, iodb, iopt, jcode, jreac, morr, morr0, mwtrc, ndac, ndact, ndctmx, nodbmx, noptmx, nord, noutpt, nrk, nrct, nrctmx, nsk, nstmax, nttyo, prcinf, prminf, qriinf, rirec1, rk, rreac1, rrelr1, rtcnst, rrxfi1, sfcar, sfcar0, ssfcar, ureac)
    end if
    ...
    ! calculations.
    call gegexw(cegexs, egexpc, egexpa, egexw, iern1, iern2, ietmax, jern1, jetmax, jgext, kern1, kern2, ketmax, kgexsa, moph, mosp, netmax, ngexsa, ngext, nptmax, nstmax, xgexw, zchar)
...
1200 format(1x)
    ...
    ! of the current run.
    call wrentu(actw, fo2lg, iopg, iopt, kstep, nopgmx, noptmx, nttyo, ph, qredox, time1, xi1)
...
110 continue
    ...
    if (qecon) then
        ...
        call chdxgz(delxi, dlxmx0, fdlim, fdzv0, iodb, jordlm, kdim, kmax, km1, kord, kxt, nodbmx, nordmx, nordz, noutpt, nrd1mx, nsscmx, scalim, scfcz, sscrew, qmin, smp100, uzvec1, zklogu, zvec0, zvclg0)
        ...
        if (iopt(2).gt.0 .and. qsawth) then
            if (kord .ge. 2) then
                ...
                call chdxtz(delxi, dlxmx0, dzvc0, iodb, jordlm, kdim, kmax, km1, kord, kxt, nodbmx, nordmx, nordzs, noutpt, nrd1mx, nsscmx, scalim, scfczs, sscrew, qmin, smp100, uzvec1, zklogu, zvec0, zvclg0)
...
125 continue
    ...
    if (nord.gt.0 .and. .not.qscon .and. iopt(3).lt.1  .and. kxt.ge.km1 .and. iopt(1).le.1) then
        ...
        ! near the boundary.
        call ldlxrd(delxi, dlxmin, dzvc0, fdzv0, iodb, ipndx1, km1, kmax, kxt, loph, nodbmx, nord, noutpt, nptmax, nrd1mx, nttyo, uphase, zklogu, zvec0)
    end if
    ...
    if (iopt(1).eq.2 .and. nord.ge.1 .and. npet.gt.0) then
        ...
        call fpbflo(delxi, demop0, dlxmin, dxval0, d1emp1, emop, emop0, eps100, fdpe0, iemop, ier, iodb, nodbmx, nord, nordmx, noutpt, npet, npetmx, nptmax, nrd1mx, nttyo, qdump, uaqsln, ufixf, uphase, xi0, xi1, xval0, zklogu)
    ...
    if (iopt(2) .gt. 0) then
        if (nrct .gt. 0) then
            ...
            call chksti(akmat0, drer0, drir0, deltim, delxi, dlxmin, fdri0, fdrr0, iodb, jreac, kly, nodbmx, kord, nord, noutpt, npts, nrct, nrctmx, nrd1mx, nttyo, prcinf, qriinf, rirec0, smp100, time0, time1, xi0, xi1)
        end if
    end if
