subroutine rd6ind(awmaxi,awmini,cbsri,cdac,cesri,cgexj,csigma,dlaplo,dlaprn,dleplo,dleprn,dlhplo,dlhprn,dloplo,dloprn,dltpll,dltplo,dltprl,dltprn,dlxdmp,dlxmx0,dlxpll,dlxplo,dlxprl,dlxprn,eact,egersi,ehmaxi,ehmini,electr,fkrc,iact,ibsrti,iesrti,ietmax,iktmax,imchmx,imech,iodb,iopg,iopr,iopt,igerti,itermx,ixrti,jcode,jgerti,jetmax,jflgi,jgext,jpress,jreac,jtemp,kbt,kct,kdim,kmax,kmt,kprs,ksplmx,ksppmx,kstpmx,kxmod,kxt,hact,modr,moffg,morr,mprphi,mprspi,mtbaqi,mtbi,mwtges,nbti,nbtmax,nbt1mx,nctmax,ndact,ndctmx,nert,nertmx,net,netmax,nffg,nffgmx,ngexrt,ninpts,nobswt,nodbmx,nopgmx,noprmx,noptmx,nordmx,noutpt,nprob,nprpmx,nprpti,nprsmx,nprsti,nptkmx,nrct,nrctmx,nrk,nsbswt,nsk,nsrt,nsrtmx,ntitl1,ntitl2,ntitmx,ntrymx,nttkmx,nttyo,nxmdmx,nxmod,nxopex,nxopmx,nxopt,nxpemx,nxrt,nxrtmx,o2maxi,o2mini,phmaxi,phmini,pressb,pressi,ptk,qend,qgexsh,qrderr,rkb,rxbari,sfcar,ssfcar,tempcb,tempci,tgexp,timmxi,tistti,tolbt,toldl,tolsat,tolxsf,trkb,ttk,ubmtbi,ubsri,ucxri,udac,uesri,uffg,ugerji,ugermo,ugersi,ugexj,ugexmo,ugexp,ugexr,uhfgex,uobsw,uprphi,uprspi,ureac,usbsw,utitl1,utitl2,uvfgex,uxcat,uxkgex,uxmod,uxopex,uxopt,uzveci,vreac,xgersi,xhfgex,ximaxi,xistti,xlkffg,xlkgex,xlkmod,xvfgex,zgexj,zvclgi)
    ...
    !! ending messages. To maintain close consistency with XCON6/rd6d8.f,
    ...
    !! EQ6/rd6inw.f, XCON6/rd6d8.f, and XCON6/rd6w8.f.
...
3000 format(/' * Error - (EQ6/rd6ind) The dimensioning parameter',' for the',/7x,'number of iodb debugging print option switches',' with string definitions',/7x,'(ndbxpa) has a value of ',i3,', but the dimensioning',/7x,'parameter for the number',' of such switches (nodbpa) has a',/7x,'value of ',i3,'.')
...
3010 format(/' * Error - (EQ6/rd6ind) The dimensioning parameter',' for the',/7x,'number of iopg activity coefficient option',' switches with string definitions',/7x,'(npgxpa) has a value',' of ' ,i3,', but the dimensioning',/7x,'parameter for the',' number of such switches (nopgpa) has a',/7x,'value of ',i3,'.')
...
3020 format(/' * Error - (EQ6/rd6ind) The dimensioning parameter',' for the',/7x,'number of iopr print option switches',' with string definitions',/7x,'(nprxpa) has a value of ',i3,', but the dimensioning',/7x,'parameter for the number',' of such switches (noprpa) has a',/7x,'value of ',i3,'.')
...
3030 format(/' * Error - (EQ6/rd6ind) The dimensioning parameter',' for the',/7x,'number of iopt model option switches',' with string definitions',/7x,'(nptxpa) has a value of ',i3,', but the dimensioning',/7x,'parameter for the number',' of such switches (noptpa) has a',/7x,'value of ',i3,'.')
...
1010 format(/' * Error - (EQ6/rd6ind) The first line of a "D"',' format input file',/7x,'should be a separator line;',' therefore, it should begin with',/7x,'"|-------". The first',' line begins instead with',/7x,'"',a,'".')
...
1015 format(/' * Error - (EQ6/rd6ind) Have too many lines in',/7x,'the main title. The code is only dimensioned for ',i4,/7x,'lines. Reduce the size of the main title or increase',/7x,'the dimensioning parameter ntitpa.')
...
1040 format(3x,'The input problem title is (first 5 lines',' maximum):',/)
        ...
        do n = 1,i
            j2 = ilnobl(utitl1(n))
            j2 = min(j2,74)
            write (nttyo,1052) utitl1(n)(1:j2)
1052 format(5x,a)
        end do
    end if

    write (nttyo,1054)
1054 format(/3x,'Continuing to read the problem input ...')
...
1020 format(/' * Error - (EQ6/rd6ind) Was expecting to find a',' line beginning with',/7x,'"',a,'", instead found one',' beginning with',/7x,'"',a,'".')
...
1022 format(/' * Error - (EQ6/rd6ind) Was expecting to find an',/7x,'option check box "[ ]" in the line beginning with',/7x,'"',a,'".')
    ...
    ! This this the mass ratio of "Fluid 2"/"Fluid 1 "at Xi = 1.
    ...
    if (icount.eq.0 .or. jtemp.lt.0) then
        write (noutpt,1023) tempcb
        write (nttyo,1023) tempcb
1023 format(/' * Warning - (EQ6/rd6ind) No option was selected for',/7x,'the temperature. The temperature will be set at a fixed',' value',/7x,'of ',f7.2,'C.')

        jtemp = 0
        ttk(1) = 0.
        ttk(2) = 0.
    else if (icount .gt. 1) then
        write (noutpt,1024) tempcb
        write (nttyo,1024) tempcb
1024 format(/' * Warning - (EQ6/rd6ind) Multiple options were',' selected for',/7x,'the temperature. The temperature will be',' set at a fixed value',/7x,'of ',f7.2,'C.')
    ...
    if (icount.eq.0 .or. jpress.lt.0) then
        write (noutpt,1030)
        write (nttyo,1030)
1030 format(/' * Warning - (EQ6/rd6ind) No option was selected for',' the pressure.',/7x,'The pressure will be set to be in',' accord with the',/7x,'data file reference pressure curve.')
        ...
        write (noutpt,1026)
        write (nttyo,1026)
1026 format(/' * Warning - (EQ6/rd6ind) Multiple options were',' selected for',/7x,'the pressure. The pressure will be set',' to be in accord with the',/7x,'data file reference pressure',' curve.')
...
1520 format(/' * Error - (EQ6/rd6ind) Have too many reactants',/7x,'The code is only dimensioned for ',i4,' reactants.',/7x,'Reduce the number of reactants or increase the',/7x,'dimensioning parameter nrctpa.')
...
1060 format(/" * Error - (EQ6/rd6ind) Don't recognize the",' jcode option string',/7x,' "',a,'". This should',' be one of the strings',/7x,'defined in the urcjco array.',' The valid strings are:',/)

        do jcox = 0,5
            j3 = ilnobl(urcjco(jcox))
            write (noutpt,1062) urcjco(jcox)(1:j3)
...
1550 format(/' * Error - (EQ6/rd6ind) Have too many solid',/7x,'solution reactants. The code is only dimensioned',/7x,'for ',i4,' such reactants. Reduce the number of',/7x,'such reactants or increase the dimensioning',' parameter nxrtpa.')
...
1590 format(/' * Error - (EQ6/rd6ind) Have too many',' end-members',/7x,'in the solid solution reactant',a,'.',/7x,'The code is only dimensioned for ',i4,' end-members per',/7x,'solid solution. Reduce',' the number of end-members or',/7x,'increase the dimensioning parameter iktpar.')
...
1600 format(/' * Error - (EQ6/rd6ind) Have too many special',/7x,'reactants. The code is only dimensioned for ',i4,/7x,'such reactants. Reduce the number of such reactants',/7x,'or increase the dimensioning parameter nsrtpa.')
...
1660 format(/' * Error - (EQ6/rd6ind) Have too many','  chemical',/7x,'elements in the special reactant ',a,'.',/7x,'The code is only dimensioned for ',i4,' elements.',/7x,'Reduce the number of elements or',' increase the',/7x,'dimensioning parameter nctpar.')
...
1700 format(/' * Error - (EQ6/rd6ind) Have too many basis',' basis species in the',/7x,'in the reaction for the',' special reactant ',a,'.',/7x,'The code is only',' dimensioned for ',i4,' basis species. Increase',/7x,'the dimensioning parameter nbtpar.')
...
1710 format(/' * Error - (EQ6/rd6ind) Have too many generic',' ion exchanger',/7x,'reactants. The code is only',' dimensioned for ',i4,' such reactants.',/7x,'Reduce',' the number of such reactants or increase the',/7x,'dimensioning parameter nertpa.')
            ...
            do jj = 1,jetmax + 1
                ...
                if (ustr(1:j2) .eq. uheadx(1:j3)) then
                    ...
                    if (jei .gt. jetmax) then
                        write (noutpt,1716) ureac(nrc)(1:j2),jetmax
                        write (nttyo,1716) ureac(nrc)(1:j2),jetmax
1716 format(/' * Error - (EQ6/rd6ind) Have too many',' exchange sites',/7x,'in the generic ion exchanger',' reactant ',a,'.',/7x,'The code is only dimensioned',' for ',i4,' exchange sites per',/7x,'generic ion',' exchanger. Reduce the number of exchange sites or',/7x,'increase the dimensioning parameter jetpar.')
...
1724 format(/' * Error - (EQ6/rd6ind) Have too many',' species on exchange',/7x,'site ',a,' of the generic',' ion exchanger reactant',/7x,a,'. The code is only',' dimensioned for species per',/7x,'exchange site.',' Reduce the number of end-members or increase',/7x,'the dimensioning parameter netpar.')
...
234 continue
        ...
        if (icount.eq.0) then
            j2 = ilnobl(ureac(nrc))
            write (noutpt,1033) ureac(nrc)(1:j2)
            write (nttyo,1033) ureac(nrc)(1:j2)
1033 format(/' * Warning - (EQ6/rd6ind) No option was selected',' for the',/7x,'surface area of the reactant ',a,'. The',/7x,'surface area and specific surface area will each be set',/7x,'to zero.')

            sfcar(nrc) = 0.0
            ssfcar(nrc) = 0.0
        else if (icount .gt. 1) then
            j2 = ilnobl(ureac(nrc))
            write (noutpt,1035) ureac(nrc)(1:j2),sfcar(nrc)
            write (nttyo,1035) ureac(nrc)(1:j2),sfcar(nrc)
1035 format(/' * Warning - (EQ6/rd6ind) Multiple options were',' selected for',/7x,'the surface area of the reactant ',a,'. The',/7x,'surface area will be set to a fixed value of ',1pe10.3,' cm3.')
...
165 continue
            j2 = ilnobl(ustr)
            write (noutpt,1075) ustr(1:j2)
            write (nttyo,1075) ustr(1:j2)
1075 format(/" * Error - (EQ6/rd6ind) Don't recognize the",' rate law option string',/7x,' "',a,'". This should',' be one of the strings',/7x,'defined in the urcnrk array.',' The valid strings are:',/)

            do jnrk = -1,3
                j3 = ilnobl(urcnrk(jnrk,jd))
                write (noutpt,1077) urcnrk(jnrk,jd)(1:j3)
...
170 continue
            ...
            if (nrk(jd,nrc) .eq. 1) then
                ...
                if (imech(jd,nrc) .gt. imchmx) then
                    j2 = ilnobl(ux8)
                    j3 = ilnobl(ureac(nrc))
                    write (noutpt,1960) ux8(1:j2),ureac(nrc)(1:j3),imchmx
                    write (nttyo,1960) ux8(1:j2),ureac(nrc)(1:j3),imchmx
1960 format(/' * Error - (EQ6/rd6ind) Have too many rate',/7x,'constants or corresponding mechanisms in the ',a,/7x,'rate law for reactant ',a,'. The code is only',/7x,'dimensioned for ',i2,' rate constants per rate law.',/7x,'Reduce the number of rate constants or increase the',/7x,'dimensioning parameter imchpa.')
                ...
                do i = 1,3
                    ...
                    if (ustr24(1:j2) .ne. uheadx(1:j3)) then
                        write (noutpt,1080) ustr24(1:j2),uheadx(1:j3)
                        write (nttyo,1080) ustr24(1:j2),uheadx(1:j3)
1080 format(/" * Error - (EQ6/rd6ind) Have an incorrect",' relative rate law header',/7x,'"',a,'".',' Was expecting the string "',a,'".')
                ...
                do ii = 1,imchmx + 1
                    ...
                    if (ustr(1:9) .ne. uheadx(1:9)) then
                        ...
                        if (imh .le. 0) then
                            j2 = ilnobl(ux8)
                            j3 = ilnobl(ureac(nrc))
                            write (noutpt,1958) ux8(1:j2),ureac(nrc)(1:j3)
                            write (nttyo,1958) ux8(1:j2),ureac(nrc)(1:j3)
1958 format(/' * Error - (EQ6/rd6ind) Have no',' "mechanisms" specified in the ',a,/7x,'rate law for reactant ',a,'.')
                    ...
                    if (icount .ne. 1) then
                        ...
                        if (icount .le. 0) then
                            write (noutpt,1118) ux8(1:j2),imh,ureac(nrc)(1:j3)
                            write (nttyo,1118) ux8(1:j2),imh,ureac(nrc)(1:j3)
1118 format(/' * Warning - (EQ6/rd6ind) No option was',' selected for',/7x,'treating the temperature',' dependence of the ',a,' rate constant',/7x,'for',' term ',i2,' of the rate equation for the reactant',' ',a,'.',/7x,'The temperature dependence will be',' set to zero.')
                        else
                            write (noutpt,1120) ux8(1:j2),imh,ureac(nrc)(1:j3)
                            write (nttyo,1120) ux8(1:j2),imh,ureac(nrc)(1:j3)
1120 format(/' * Warning - (EQ6/rd6ind) Multiple options',' were selected for',/7x,'treating the temperature',' dependence of the ',a,' rate constant',/7x,'for',' term ',i2,' of the rate equation for the reactant',' ',a,'.',/7x,'The temperature dependence will be',' set to zero.')
                        end if
                    ...
                    do jj = 1,ndctmx + 1
                        ...
                        if (ndt .gt. ndctmx) then
                            j2 = ilnobl(ux8)
                            j3 = ilnobl(ureac(nrc))
                            write (noutpt,1130) i,ux8(1:j2),ureac(nrc)(1:j3),ndctmx
                            write (nttyo,1130) i,ux8(1:j2),ureac(nrc)(1:j3),ndctmx
1130 format(/' * Error - (EQ6/rd6ind) Have too many',/7x,'species in the activity product in term ',i2,/7x,'of the ',a,' direction rate law for reactant',/7x,a,'. The code is only dimensioned for ',i3,/7x,'such species. Reduce the number of such species',/7x,'or increase the dimensioning parameter ndctpa.')
...
175 continue
                ...
                do ii = 1,imchmx + 1
                    ...
                    if (imh .gt. 1) then
                        j2 = ilnobl(ux8)
                        j3 = ilnobl(ureac(nrc))
                        write (noutpt,1964) ux8(1:j2),ureac(nrc)(1:j3)
                        write (nttyo,1964) ux8(1:j2),ureac(nrc)(1:j3)
1964 format(/' * Error - (EQ6/rd6ind) Have more than the',' allowed one "mechanism"',/7x,'in the ',a,' rate','law for reactant ',a,'.')
                    ...
                    if (icount .ne. 1) then
                        ...
                        if (icount .le. 0) then
                            write (noutpt,1118) ux8(1:j3),imh,ureac(nrc)(1:j2)
                            write (nttyo,1118) ux8(1:j3),imh,ureac(nrc)(1:j2)
                        else
                            write (noutpt,1120) ux8(1:j3),imh,ureac(nrc)(1:j2)
                ...
                j3 = ilnobl(ux8)
                j2 = ilnobl(ureac(nrc))
                write (noutpt,1952) ux8(1:j3),nrk(jd,nrc),ureac(nrc)(1:j2)
                write (nttyo,1952) ux8(1:j3),nrk(jd,nrc),ureac(nrc)(1:j2)
1952 format(/' * Error - (EQ6/rd6ind) Programming error trap:',' The ',a,/7x,'rate law code has an unrecognized value',' of ',i2,' for reactant',/7x,a,'.')
...
3170 format(a)
    ...
    do jd = 1,2
        ...
        call locase(ux24)
        j2 = ilnobl(ux24)
        write (noutpt,3190) ux24(1:j2),jd
...
3210 format(a)
    ...
    do nn = 1,noptmx
        ...
        if (k1.le.0 .or. k2.le.0 .or. k2.lt.(k1 + 2) .or. k3.le.0 .or.  k3.le.k2) then
            write (noutpt,1607) uheadx(1:j2)
            write (nttyo,1607) uheadx(1:j2)
1607 format(/' * Error - (EQ6/rd6ind) The iopt option switch',' title line',/7x,'"',a,'"',/7x,'read from the'," input file isn't in the required format.")
...
1610 format(/' * Error - (EQ6/rd6ind) The iopt option switch',' title line',/7x,'"',a,'"',/7x,'read from the',' input file references an option switch index',/7x,'of ',i3,', which is out of range.')
...
1612 format(/' * Error - (EQ6/rd6ind) The iopt option switch',' title string',/7x,'"',a,'"',/7x,"read from the input file doesn't contain the",' matching defined string',/7x,'"',a,'".')
...
1630 format(/' * Error - (EQ6/rd6ind) Have too many option',/7x,'choice lines for the iopt option switch whose title',' string is',/7x,'"',a,'".',/7x,' The code is only',' dimensioned for ',i3,' such lines. Reduce the',/7x,'number of such lines or increase the dimensioning',' parameter jptxpa.')
...
1640 format(/' * Error - (EQ6/rd6ind) The following iopt',/7x,'option switch choice line read from the input file',/7x,"isn't in the required format:",/7x,'"',a,'"')
...
1650 format(/' * Error - (EQ6/rd6ind) The iopt option switch',/7x,'choice line',/7x,'"',a,'"',/7x,'read from the input file references an out-of-range',' option',/7x,'choice index.')
...
600 continue
            ...
            if (index(uheadx(1:j2),ustr(1:j3)) .le. 0) then
                j4 = ilnobl(uopttx(n))
                write (noutpt,1662) uheadx(1:j2),ustr(1:j3),uopttx(n)(1:j4)
                write (nttyo,1662) uheadx(1:j2),ustr(1:j3),uopttx(n)(1:j4)
1662 format(/' * Error - (EQ6/rd6ind) The iopt option switch',' choice line',/7x,'"',a,'"',/7x," read from the input file doesn't contain the",' matching defined string',/7x,'"',a,'".',/7x,'This line belongs to the option switch whose title',' string is',/7x,'"',a,'".')
...
1670 format(/" * Error - (EQ6/rd6ind) Don't recognize the",' string "',a,'"',/7x,'that appears on the iopt',' option switch choice line',/7x,'"',a,'"',/7x,'read from the input file. An option choice should',' be chosen by',/7x,'placing a "*", "x", or "X" in the',' checkbox ("[ ]"). This',/7x,'choice line belongs to',' the option switch whose title string is',/7x,'"',a,'".')
...
1680 format(/' * Warning - (EQ6/rd6ind) No option choice was',' checked on the input file',/7x,'for the iopt option',' switch whose title string is',/7x,'"',a,'".')
...
1694 format(/' * Warning - (EQ6/rd6ind) More than one option',' choice was checked',/7x,'on the input file for the iopt',' option switch whose title string is',/7x,'"',a,'".',/7x,'The last choice checked will be used. The',' matching string is',/7x,'"',a,'".')
        end if
    end do
...
1800 format(/' * Error - (EQ6/rd6ind) The iopr option switch',' title line',/7x,'"',a,'"',/7x,'read from the'," input file isn't in the required format.")
...
1810 format(/' * Error - (EQ6/rd6ind) The iopr option switch',' title line',/7x,'"',a,'"',/7x,'read from the',' input file references an option switch index',/7x,'of ',i3,', which is out of range.')
...
1812 format(/' * Error - (EQ6/rd6ind) The iopr option switch',' title string',/7x,'"',a,'"',/7x,"read from the input file doesn't contain the",' matching defined string',/7x,'"',a,'".')
...
1830 format(/' * Error - (EQ6/rd6ind) Have too many option',/7x,'choice lines for the iopr option switch whose title',' string is',/7x,'"',a,'".',/7x,' The code is only',' dimensioned for ',i3,' such lines. Reduce the',/7x,'number of such lines or increase the dimensioning',' parameter jprxpa.')
...
1840 format(/' * Error - (EQ6/rd6ind) The following iopr',/7x,'option switch choice line read from the input file',/7x,"isn't in the required format:",/7x,'"',a,'"')
...
1850 format(/' * Error - (EQ6/rd6ind) The iopr option switch',/7x,'choice line',/7x,'"',a,'"',/7x,'read from the input file references an out-of-range',' option',/7x,'choice index.')
...
1860 format(/' * Error - (EQ6/rd6ind) The iopr option switch',' choice line',/7x,'"',a,'"',/7x," read from the input file doesn't contain the",' matching defined string',/7x,'"',a,'".',/7x,'This line belongs to the option switch whose title',' string is',/7x,'"',a,'".')
...
1870 format(/" * Error - (EQ6/rd6ind) Don't recognize the",' string "',a,'"',/7x,'that appears on the iopr',' option switch choice line',/7x,'"',a,'"',/7x,'read from the input file. An option choice should',' be chosen by',/7x,'placing a "*", "x", or "X" in the',' checkbox ("[ ]"). This',/7x,'choice line belongs to',' the option switch whose title string is',/7x,'"',a,'".')
...
1880 format(/' * Warning - (EQ6/rd6ind) No option choice was',' checked on the input file',/7x,'for the iopr option',' switch whose title string is',/7x,'"',a,'".')
...
1894 format(/' * Warning - (EQ6/rd6ind) More than one option',' choice was checked',/7x,'on the input file for the iopr',' option switch whose title string is',/7x,'"',a,'".',/7x,'The last choice checked will be used. The',' matching string is',/7x,'"',a,'".')
        end if
    end do
...
1900 format(/' * Error - (EQ6/rd6ind) The iodb option switch',' title line',/7x,'"',a,'"',/7x,'read from the'," input file isn't in the required format.")
...
1910 format(/' * Error - (EQ6/rd6ind) The iodb option switch',' title line',/7x,'"',a,'"',/7x,'read from the',' input file references an option switch index',/7x,'of ',i3,', which is out of range.')
...
1920 format(/' * Error - (EQ6/rd6ind) The iodb option switch',' title string',/7x,'"',a,'"',/7x,"read from the input file doesn't contain the",' matching defined string',/7x,'"',a,'".')
...
1922 format(/' * Error - (EQ6/rd6ind) Have too many option',/7x,'choice lines for the iodb option switch whose title',' string is',/7x,'"',a,'".',/7x,' The code is only',' dimensioned for ',i3,' such lines. Reduce the',/7x,'number of such lines or increase the dimensioning',' parameter jdbxpa.')
...
1940 format(/' * Error - (EQ6/rd6ind) The following iodb',/7x,'option switch choice line read from the input file',/7x,"isn't in the required format:",/7x,'"',a,'"')
...
1950 format(/' * Error - (EQ6/rd6ind) The iodb option switch',/7x,'choice line',/7x,'"',a,'"',/7x,'read from the input file references an out-of-range',' option',/7x,'choice index.')
...
900 continue
            ...
            if (index(uheadx(1:j2),ustr(1:j3)) .le. 0) then
                j4 = ilnobl(uodbtx(n))
                write (noutpt,1962) uheadx(1:j2),ustr(1:j3),uodbtx(n)(1:j4)
                write (nttyo,1962) uheadx(1:j2),ustr(1:j3),uodbtx(n)(1:j4)
1962 format(/' * Error - (EQ6/rd6ind) The iodb option switch',' choice line',/7x,'"',a,'"',/7x," read from the input file doesn't contain the",' matching defined string',/7x,'"',a,'".',/7x,'This line belongs to the option switch whose title',' string is',/7x,'"',a,'".')
...
1970 format(/" * Error - (EQ6/rd6ind) Don't recognize the",' string "',a,'"',/7x,'that appears on the iodb',' option switch choice line',/7x,'"',a,'"',/7x,'read from the input file. An option choice should',' be chosen by',/7x,'placing a "*", "x", or "X" in the',' checkbox ("[ ]"). This',/7x,'choice line belongs to',' the option switch whose title string is',/7x,'"',a,'".')
...
1980 format(/' * Warning - (EQ6/rd6ind) No option choice was',' checked on the input file',/7x,'for the iodb option',' switch whose title string is',/7x,'"',a,'".')
...
1994 format(/' * Warning - (EQ6/rd6ind) More than one option',' choice was checked',/7x,'on the input file for the iodb',' option switch whose title string is',/7x,'"',a,'".',/7x,'The last choice checked will be used. The',' matching string is',/7x,'"',a,'".')
        end if
    end do
...
1320 format(/' * Error - (EQ6/rd6ind) Have too many mineral',/7x,'subset-selection suppression options. The code is',/7x,'only dimensioned for ',i3,' such options. Reduce the',/7x,'number of options or increase the dimensioning',/7x,'parameter nxoppa.')
        ...
        j2 = ilnobl(ustr)
        write (noutpt,3060) ustr(1:j2)
        write (nttyo,3060) ustr(1:j2)
3060 format(/" * Error - (EQ6/rd6ind) Don't recognize the",' uxopt option string',/7x,'"',a,'". This should',' be one of the strings',/7x,'defined in the uxopti array.',' The valid strings are:',/)

        do n = 1,4
            j3 = ilnobl(uxopti(n))
            write (noutpt,3062) uxopti(n)(1:j3)
...
1370 format(/' * Error - (EQ6/rd6ind) Have too many',/7x,'exceptions specified to the mineral subset-selection',/7x,'suppression options. The code is only dimensioned',/7x,'for ',i3,'exceptions. Reduce the number of exceptions',/7x,'or increase the dimensioning parameter nxpepa.')
...
1420 format(/' * Error - (EQ6/rd6ind) Have too many gases whose',' fugacities are to be fixed.',/7x,'The code is only',' dimensioned for ',i4,' such gases. Reduce the number',/7x,'of gases or increase the dimensioning parameter nffgpa.')
...
7170 format('* Start of the bottom half of the input file',t80,'*')
    ...
    do nn = 1,ntitmx + 1
        ...
        if (n .gt. ntitmx) then
            write (noutpt,5015) ntitmx
            write (nttyo,5015) ntitmx
5015 format(/' * Error - (EQ6/rd6ind) Have too many lines in',/7x,'the secondary title. The code is only dimensioned for ',i4,/7x,'lines. Reduce the size of the secondary title or ','increase',/7x,'the dimensioning parameter ntitpa.')
...
1210 format(/' * Error - (EQ6/rd6ind) Was expecting to find the',/7x,'string "',a,'" at the start of the line beginning with',/7x,'"',a,'".')
...
1220 format(/' * Warning - (EQ6/rd6ind) Could not interpret the',' advisory on whether',/7x,'or not one or more exchanger',' blocks follow. The problem is with',/7x,'the line',' beginning with',/7x,'"',a,'".',/7x,'The presence of such',' blocks will be determined directly.')
        end if
    end if
...
1230 format(/' * Error - (EQ6/rd6ind) Was expecting to find an',/7x,'option check box "[ ]" in the line beginning with',/7x,'"',a,'"',/7x,'The expected line contains the box for',' the qgexsh flag.')
...
1240 format(/' * Error - (EQ6/rd6ind) The qgexsh flag line',/7x,'"',a,'"',/7x,"read from the input file doesn't contain",' the expected string',/7x,'"',a,'".')
    ...
    if (j2 .gt. 0) then
        if (index(ustr24,'*') .ge. 1) then
            ...
            write (noutpt,1250) ustr24(1:j2),uline1(1:j3)
1250 format(/" * Error - (EQ6/rd6ind) Don't recognize the",' string "',a,'"',/7x,'that appears on the qgexsh',' option switch choice line',/7x,'"',a,'"',/7x,'read from the input file. An option choice should',' be chosen by',/7x,'placing a "*", "x", or "X" in the',' checkbox ("[ ]").')
    ...
    if (ustr(1:j2) .eq. uheadx(1:j3)) then
        ...
        if (qgexbs .and. .not.qgexbf) then
            write (nttyo,1260)
            write (noutpt,1260)
1260 format(/' * Note - (EQ6/rd6ind) The advisory on the',' presence of',/7x,'ion exchanger blocks was negative,',' but one or more blocks is',/7x,'actually present.')
        end if
    else
        ...
        if (qgexbs .and. qgexbf) then
            write (nttyo,1270)
            write (noutpt,1270)
1270 format(/' * Note - (EQ6/rd6ind) The advisory on the',' presence of',/7x,'ion exchanger blocks was positive,',' but no blocks are actually',/7x,'present.')
        end if
    end if
...
1620 format(/' * Error - (EQ6/rd6ind) Have exceeded the',' maximum number of ',i3,/7x,'generic ion exchange phases',' while reading the data to create',/7x,'such phases.',' Increase the dimensioning parameter netpar',/7x,'to at',' least ',i3,'.')
        ...
        if (ustr24(1:8) .ne. uheadx(1:8)) then
            ...
            write (noutpt,1070) uline1(1:j2)
            write (nttyo,1070) uline1(1:j2)
1070 format(/' * Error - (EQ6/rd6ind) Read the following line',' where a separator line',/7x,'("|------- ... -------|")',' was expected:',/7x,'"',a,'".')
            ...
            if (ustr(1:j2) .eq. uheadx(1:j3)) then
                ...
                if (qokay) then
                    ...
                    if (je .gt. jetmax) then
                        j2 = ilnobl(ugexp(ne))
                        write (noutpt,1290) jetmax,ugexp(ne)(1:j2),je
                        write (nttyo,1290) jetmax,ugexp(ne)(1:j2),je
1290 format(/' * Error - (EQ6/rd6ind) Have exceeded the',' maximum number of ',i3,/7x,'exchange sites on a',' generic ion exchange phase while reading',/7x,'the',' data to create ',a,'. Increase the',/7x,'dimensioning parameter jetpar to at least ',i3,'.')
...
1820 format(/' * Error - (EQ6/rd6ind) Have exceeded the',' maximum number of ',i3,/7x,'reactions for a site',' belonging to a generic ion exchange',/7x,'phase',' while reading the data for site ',a,' of exchange',' phase',/7x,a,'. Increase the dimensioning',' parameter',/7x,'ietpar to at least ',i3,'.')
...
570 continue
    ...
    ! Add comment lines listing valid option strings.
    write (noutpt,2694)
2694 format('* Valid exchange model strings (ugexmo(n)) are:',t80,'*')
...
2696 format(a)
    ...
    write (noutpt,2690)
2690 format('* Valid units strings (uxkgex(i,j,n)/uhfgex(i,j,n)/','uvfgex(i,j,n)) are:',t80,'*')
...
1500 format(/' * Error - (EQ6/rd6ind) Have too many nxmod',/7x,'alter/suppress options. The code is only dimensioned',/7x,'for ',i3,' such options. Reduce the number of such',/7x,'options or increase the dimensioning parameter nxmdpa.')
        ...
        j2 = ilnobl(ustr)
        write (noutpt,1510) ustr(1:j2)
        write (nttyo,1510) ustr(1:j2)
1510 format(/" * Error - (EQ6/rd6ind) Don't recognize the",' alter/suppress option',/7x,'string "',a,'". This should',' be one of the strings',/7x,'defined in the ukxm array.',' The valid strings are:',/)

        do kxmd = -1,2
            j3 = ilnobl(ukxm(kxmd))
            write (noutpt,1514) ukxm(kxmd)(1:j3)
...
5094 format(a)
    ...
    do nn = 1,nopgmx
        ...
        if (k1.le.0 .or. k2.le.0 .or. k2.lt.(k1 + 2) .or. k3.le.0 .or.  k3.le.k2) then
            write (noutpt,1705) uheadx(1:j2)
            write (nttyo,1705) uheadx(1:j2)
1705 format(/' * Error - (EQ6/rd6ind) The iopg option switch',' title line',/7x,'"',a,'"',/7x,'read from the'," input file isn't in the required format.")
...
1712 format(/' * Error - (EQ6/rd6ind) The iopg option switch',' title line',/7x,'"',a,'"',/7x,'read from the',' input file references an option switch index',/7x,'of ',i3,', which is out of range.')
...
1720 format(/' * Error - (EQ6/rd6ind) The iopg option switch',' title string',/7x,'"',a,'"',/7x,"read from the input file doesn't contain the",' matching defined string',/7x,'"',a,'".')
...
1730 format(/' * Error - (EQ6/rd6ind) Have too many option',/7x,'choice lines for the iopg option switch whose title',' string is',/7x,'"',a,'".',/7x,' The code is only',' dimensioned for ',i3,' such lines. Reduce the',/7x,'number of such lines or increase the dimensioning',' parameter jpgxpa.')
...
1740 format(/' * Error - (EQ6/rd6ind) The following iopg',/7x,'option switch choice line read from the input file',/7x,"isn't in the required format:",/7x,'"',a,'"')
...
1750 format(/' * Error - (EQ6/rd6ind) The iopg option switch',/7x,'choice line',/7x,'"',a,'"',/7x,'read from the input file references an out-of-range',' option',/7x,'choice index.')
...
1760 format(/' * Error - (EQ6/rd6ind) The iopg option switch',' choice line',/7x,'"',a,'"',/7x," read from the input file doesn't contain the",' matching defined string',/7x,'"',a,'".',/7x,'This line belongs to the option switch whose title',' string is',/7x,'"',a,'".')
...
1770 format(/" * Error - (EQ6/rd6ind) Don't recognize the",' string "',a,'"',/7x,'that appears on the iopg',' option switch choice line',/7x,'"',a,'"',/7x,'read from the input file. An option choice should',' be chosen by',/7x,'placing a "*", "x", or "X" in the',' checkbox ("[ ]"). This',/7x,'choice line belongs to',' the option switch whose title string is',/7x,'"',a,'".')
...
1780 format(/' * Warning - (EQ6/rd6ind) No option choice was',' checked on the input file',/7x,'for the iopg option',' switch whose title string is',/7x,'"',a,'".')
...
1794 format(/' * Warning - (EQ6/rd6ind) More than one option',' choice was checked',/7x,'on the input file for the iopg',' option switch whose title string is',/7x,'"',a,'".',/7x,'The last choice checked will be used. The',' matching string is',/7x,'"',a,'".')
        end if
    end do
...
3240 format(/' * Error - (EQ6/rd6ind) Have too many chemical',/7x,'elements present. The code is only dimensioned',/7x,'for ',i3,' elements. Reduce the number of elements',/7x,'or increase the dimensioning parameter nctpar.')
...
3250 format(/' * Error - (EQ6/rd6ind) Have too many basis',/7x,'species present. The code is only dimensioned',/7x,'for ',i3,' basis species. Reduce the number of elements',/7x,'or increase the dimensioning parameter nctpar.')
...
3260 format(/' * Error - (EQ6/rd6ind) Have too many matrix',/7x,'variables. The code is only dimensioned for ',i3,/7x,'matrix variables. Reduce the number of such variables',/7x,'or increase the dimensioning parameter kpar.')
...
910 continue
    ...
    if (nfldt .ne. nfldtx) then
        j2 = ilnobl(uline1)
        j2 = min(j2,70)
        write (noutpt,5010) nfldt,nfldtx,uline1(1:j2)
        write (nttyo,5010) nfldt,nfldtx,uline1(1:j2)
5010 format(/' * Error - (EQ6/rd6ind) Found ',i2,' fields',' where ',i2,/7x,'were expected on the line which begins with',/7x,'"',a,'".')
    ...
    if (nbi .gt. nbtmax) then
        j2 = ilnobl(ux48)
        write (noutpt,5020) nbtmax,ux48(1:j2)
        write (nttyo,5020) nbtmax,ux48(1:j2)
5020 format(/' * Error - (EQ6/rd6ind) The number of mass balance',' species read',/7x,'from the input file exceeded the',' maximum of ',i3,' while',/7x,'trying to read data for',' the species ',a,'.',/7x,'Increase the dimensioning',' parameter nbtpar.')
    ...
    if (ustrn(1:16) .eq. 'moles           ') then
        ...
        j2 = ilnobl(ustr)
        write (noutpt,5030) ustr(1:j2)
        write (nttyo,5030) ustr(1:j2)
5030 format(/" * Error - (EQ6/rd6ind) Can't identify the",/7x,'following Mass-Balance-Species Units/constraints string- ',a,'.',/7x,'This must be one of "Moles", or "Make non-basis".')
...
7830 format(a)
    ...
    do nbi = 1,nbti
        ...
        if (ustr(1:32) .ne. ux48(1:32)) then
            write (noutpt,5040) ux48(1:j3),ustr(1:j2)
            write (nttyo,5040) ux48(1:j3),ustr(1:j2)
5040 format(/' * Error - (EQ6/rd6ind) Found that the mass',' balance species name',/7x,'"',a,'".',/7x,"doesn't match",' the mass balance totals species name tag',/7x,'"',a,'"',/7x,'(which has a maximum length of only 32 characters).')
...
960 continue
    ...
    if (krow .gt. kmax) then
        j2 = ilnobl(ux48)
        write (noutpt,5050) kmax,ux48(1:j2)
        write (nttyo,5050) kmax,ux48(1:j2)
5050 format(/' * Error - (EQ6/rd6ind) The number of matrix',' column variables',/7x,'and values read from the input file',' exceeded the maximum',/7x,'of ',i3,' while trying to read',' data for the basis species',/7x,a,'. Increase the',' dimensioning parameter kmax.')
...
3640 format(/' * Error - (EQ6/rd6ind) Have too many phases',/7x,'in the physically removed system (PRS) on the',/7x,'input file. The code is only dimensioned for ',i3,/7x,'such phases. Reduce the number of such phases',/7x,'or increase the dimensioning parameter nprppa.')
            ...
            if (ustr(1:8) .ne. uheadx(1:8)) then
                ...
                if (qokay) then
                    ...
                    if (iki .gt. iktmax) then
                        ...
                        write (noutpt,3680) uprphi(npi)(1:j2),iktmax
                        write (nttyo,3680) uprphi(npi)(1:j2),iktmax
3680 format(/' * Error - (EQ6/rd6ind) Have too many',' end-members',/7x,'in the PRS solid solution ',a,'.',/7x,'The code is only dimensioned for ',i4,' end-members per',/7x,'solid solution. Reduce',' the number of end-members or',/7x,'increase the dimensioning parameter iktpar.')
                    ...
                    if (nsi .gt. nprsmx) then
                        write (noutpt,2000) nprsmx,nsi
                        write (nttyo,2000) nprsmx,nsi
2000 format(/' * Error - (EQ6/rd6ind) Have exceeded the',' maximum number of ',i3,/7x,'species while reading',' the data for phases and species in the PRS',/7x,' Increase the dimensioning parameter',' nprsmx to at least ',i3,'.')
...
340 continue
    ...
    write (noutpt,3300) nprob
    write (nttyo,3300) nprob
3300 format(/'   Done reading problem ',i3,'.',/)
