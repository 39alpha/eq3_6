...
100 continue
    ...
    if (icount .gt. 10) then
        if (iodb(1).gt.0 .or. iodb(5).gt.0) then
            write (noutpt,1000)
1000 format(/3x,'A scan for where the Eh reaches the requested',' maximum value',/3x,'has failed. Dropping to the minimum',' step size.')
        end if
...
1020 format(/3x,"Taylor's series predict that the Eh is",' at the requested',/3x,'maximum value of ',1pe11.4,' v at Xi= ',e11.4,',',/3x,'delxi= ',e11.4,'.')
        end if
    end if
