      subroutine wr3d7(cspb,fep,iktmax,iodb,iopg,iopr,iopt,itermx,
...
c
      ...
      data udef(4)  / '30              '/
c
c     data udef(1)  / '1.0e-10  tolbt  '/
c     data udef(2)  / '1.0e-10  toldl  '/
c     data udef(3)  / '0.5      tolsat '/
c     data udef(4)  / '30       itermx '/
c
      data ujx(0)   / 'aqueous         '/
      data ujx(1)   / 'mineral         '/
      data ujx(2)   / 'gas             '/
...
c
 ...
 1080 format('|Electrical Balancing on |',a14,'|',a1,'code selects  |',
     $ a1,'not performed|')
      write (newin,1090)
c
c-----------------------------------------------------------------------
...
c
      write (newin,1110)
 1110 format('|SPECIES   | BASIS SWITCH/CONSTRAINT |',
     $ ' CONCENTRATION| UNITS OR TYPE    |')
      write (newin,1090)
c
      if (nsq .le. 0) then
        write (newin,1120)
...
c
      ...
      if (iopt(1).eq.1 .or. iopt(1).eq.-3) then
        write (newin,1140)
1140    format('|redox',t12,'| Defined by next species',t38,'|',t53,'|',
     $  'Redox couple',t72,'|')
        do 120 ns = 1,nsq
...
c
 ...
 1145   format(" * Error - (XCON3/wr3d7) Can't find the redox",
...
c
 ...
 1160     format(" * Error - (XCON3/wr3d7) Can't find the solid",
...
c
 ...
 1170     format(" * Error - (XCON3/wr3d7) Can't find the end-member",
...
c
      write (newin,1480)
 1480 format('|DEBUGGING SWITCHES (o-off, 1,2-on, default is off)',
     $ t72,'|')
      write (newin,1090)
c
