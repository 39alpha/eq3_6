subroutine rd3d8b(cgexj,cgexpi,covali,ehi,egexsi,fo2lgi,iebal3,ietmax,iodb,iopg,iopr,iopt,irdxc3,itdsf3,itermx,jgext,jetmax,jflgi,jgexti,jpres3,kxmod,mwtges,nbti,nbtmax,ncmpri,net,neti,netmax,ngexti,ninpts,ngexrt,nobswt,nodbmx,nopgmx,noprmx,noptmx,noutpt,nprob,nsbswt,ntitl,ntitmx,nttyo,nxicmx,nxmdmx,nxmod,nxti,nxtimx,pei,press,qend,qrderr,rho,scamas,tdspkg,tdspl,tempc,tgexp,tolbt,toldl,tolspf,ucospi,uebal,ugexj,ugexji,ugexmo,ugexp,ugexpi,ugexr,ugexsi,umemi,uobsw,uredox,usbsw,usoli,uspeci,utitl,uhfgex,uvfgex,uxkgex,uxmod,xbari,xgexsi,xhfgex,xlkgex,xvfgex,xlkmod,zgexj)
...
3000 format(/' * Error - (XCON3/rd3d8b) The dimensioning parameter',' for the',/7x,'number of iodb debugging print option switches',' with string definitions',/7x,'(ndbxpa) has a value of ',i3,', but the dimensioning',/7x,'parameter for the number',' of such switches (nodbpa) has a',/7x,'value of ',i3,'.')
...
3010 format(/' * Error - (XCON3/rd3d8b) The dimensioning parameter',' for the',/7x,'number of iopg activity coefficient option',' switches with string definitions',/7x,'(npgxpa) has a value',' of ' ,i3,', but the dimensioning',/7x,'parameter for the',' number of such switches (nopgpa) has a',/7x,'value of ',i3,'.')
...
3020 format(/' * Error - (XCON3/rd3d8b) The dimensioning parameter',' for the',/7x,'number of iopr print option switches',' with string definitions',/7x,'(nprxpa) has a value of ',i3,', but the dimensioning',/7x,'parameter for the number',' of such switches (noprpa) has a',/7x,'value of ',i3,'.')
...
3030 format(/' * Error - (XCON3/rd3d8b) The dimensioning parameter',' for the',/7x,'number of iopt model option switches',' with string definitions',/7x,'(nptxpa) has a value of ',i3,', but the dimensioning',/7x,'parameter for the number',' of such switches (noptpa) has a',/7x,'value of ',i3,'.')
...
1010 format(/' * Error - (XCON3/rd3d8b) The first line of a "D"',' format input file',/7x,'should be a separator line;',' therefore, it should begin with',/7x,'"|-------". The first',' line begins instead with',/7x,'"',a,'".')
...
1015 format(/' * Error - (XCON3/rd3d8b) Have too many lines in',/7x,'the title. The code is only dimensioned for ',i4,/7x,'lines. Reduce the size of the title or increase the',/7x,'dimensioning parameter ntitpa.')
...
1020 format(/' * Error - (XCON3/rd3d8b) Was expecting to find a',' line beginning with',/7x,'"',a,'", instead found one',' beginning with',/7x,'"',a,'".')
...
1022 format(/' * Error - (XCON3/rd3d8b) Was expecting to find an',/7x,'option check box "[ ]" in the line beginning with',/7x,'"',a,'".')
    ...
    if (icount .eq. 0) then
        write (nttyo,1024)
1024 format(/' * Note - (XCON3/rd3d8b) No option was selected for',' the pressure.',/7x,'The pressure will be set to be in',' accord with the',/7x,'data file reference pressure curve.')

        jpres3 = 0
        press = 0.
    else if (icount .gt. 1) then
        write (nttyo,1026)
1026 format(/' * Warning - (XCON3/rd3d8b) Multiple options were',' selected for',/7x,'the pressure. The pressure will be set',' to be in accord with the',/7x,'data file reference pressure',' curve.')
    ...
    if (icount .eq. 0) then
        write (nttyo,1034)
1034 format(/' * Note - (XCON3/rd3d8b) No option was selected',' for the total',/7x,'dissolved solutes (TDS). The TDS will',' be set to 0 mg/kg.sol.')
...
1036 format(/' * Warning - (XCON3/rd3d8b) Multiple options were',' selected for the',/7x,'total dissolved solutes (TDS).',' The TDS will be set to',/7x,g12.5,' mg/kg.sol.')
    ...
    if (icount .eq. 0) then
        write (nttyo,1044)
1044 format(/' * Note - (XCON3/rd3d8b) No electrical balancing',' option was',/7x,'selected. No balancing will be done.')

        iebal3 = 0
        uebal = 'None'
    else if (icount .gt. 1) then
        write (nttyo,1046)
1046 format(/' * Warning - (XCON3/rd3d8b) Multiple electrical',' balancing options.',/7x,'were selected. No balancing',' will be done.')
...
1047 format(/' * Note - (XCON3/rd3d8b) No default redox constraint',' option was',/7x,'selected. A default condition of log fO2= ',g12.5,' (log bars) will be used.')

        irdxc3 = 0
    else if (icount .gt. 1) then
        write (nttyo,1048)
1048 format(/' * Warning - (XCON3/rd3d8b) Multiple default redox',' constraint options.',/7x,'were selected. The last option',' that was selected will be used.')
    end if
...
1030 format(/' * Warning - (XCON3/rd3d8b) Found ',i2,' fields',' where ',i2,/7x,'were expected on the line which begins with',/7x,'"',a,'".')
    end if
...
1050 format(/' * Error - (XCON3/rd3d8b) The number of basis',' species read',/7x,'from the input file exceeded the',' maximum of ',i3,' while',/7x,'trying to read data for',' the species ',a,'.',/7x,'Increase the dimensioning',' parameter nbtpar.')
...
1060 format(/" * Error - (XCON3/rd3d8b) Don't recognize the",' jflag option string',/7x,' "',a,'". This should',' be one of the strings defined in the',/7x,'ujf3 array.')
...
1620 format(/' * Error - (XCON3/rd3d8b) Have exceeded the',' maximum number of ',i3,/7x,'generic ion exchange phases',' while reading the data to create',/7x,'such phases.',' Increase the dimensioning parameter netpar',/7x,'to at',' least ',i3,'.')
...
1070 format(/' * Error - (XCON3/rd3d8b) Read the following line',' where a separator line',/7x,'("|------- ... -------|")',' was expected:',/7x,'"',a,'".')
...
1710 format(/' * Error - (XCON3/rd3d8b) Have exceeded the',' maximum number of ',i3,/7x,'exchange sites on a',' generic ion exchange phase while reading',/7x,'the',' data to create ',a,'. Increase the',/7x,'dimensioning parameter jetpar to at least ',i3,'.')
...
1820 format(/' * Error - (XCON3/rd3d8b) Have exceeded the',' maximum number of ',i3,/7x,'reactions for a site',' belonging to a generic ion exchange',/7x,'phase',' while reading the data for site ',a,' of exchange',' phase',/7x,a,'. Increase the dimensioning',' parameter',/7x,'ietpar to at least ',i3,'.')
...
1930 format(/' * Error - (XCON3/rd3d8b) Have exceeded the',' maximum number of ',i3,/7x,'generic ion exchange phases',' while reading the data for concentrations',7x,'and',' compositions of such phases. Increase the dimensioning',/7x,'parameter netpar to at least ',i3,'.')
...
1932 format(/' * Error - (XCON3/rd3d8b) Data are present on the',' input file for the',/7x,'generic ion exchange phase "',a,'", but this phase',/7x,"hasn't been previously defined."," Can't determine the requisite model,",/7x,"therefore can't",' finish reading the current data for this phase.')
...
2000 format(/' * Error - (XCON3/rd3d8b) Have exceeded the',' maximum number of ',i3,/7x,'exchange sites on a',' generic ion exchange phase while reading',/7x,'the',' data for the concentration and composition of',/7x,a,'. Increase the dimensioning parameter jetpar',/7x,'to at least ',i3,'.')
...
2010 format(/' * Error - (XCON3/rd3d8b) Was expecting to find',' a line containing',/7x,'"',a,'", instead found one',' containing "',a,'".')
...
2040 format(/' * Error - (XCON3/rd3d8b) Have exceeded the',' maximum number of ',i3,/7x,'species on an exchange',' site of a generic ion exchange phase',/7x,'while',' reading',/7x,'the data for the composition of',' site ',a,' of',/7x,'exchange phase ',a,'. Increase',' the dimensioning parameter ietpar to at least ',i3,'.')
...
2014 format(/' * Warning - (XCON3/rd3d8b) Was expecting',' to find a line containing',/7x,'"',a,'", instead',' found one containing "',a,'".',/7x,'The former',' matches the ',a,' exchange model, which was',/7x,'specified for the generic ion exchange phase ',a,'.')
                        end if
...
1360 format(/' * Error - (XCON3/rd3d8b) Have unrecognized',' input on the line',/7x,'beginning with "',a,'".',' This line should contain the',/7x,'name of an',' exchanger species and corresponding mole fraction',/7x,'for site ',a,' of exchanger phase ',a,', else it',/7x,'should be a separator line terminating a sequence',' of such lines.')
                    end if
                end if
            end do
...
1430 format(/' * Error - (XCON3/rd3d8b) Have exceeded the',' maximum',/7x,'number of ',i5,' solid solutions for which',/7x,'compositions may be specified on the input file. This',/7x,'occurred while reading data for ',a,'.',/7x,'Increase the dimensioning parameter nxtipa.')
...
1460 format(/' * Error - (XCON3/rd3d8b) Have exceeded the',' maximum number',/7x,'of ',i5,' solid solution',' species for which mole fractions',/7x,'are specified',' on the input file. This occurred while reading',/7x,'data for the species ',a,'(',a,').',/7x,'Increase the dimensioning parameter nxipar.')
...
1470 format(/' * Error - (XCON3/rd3d8b) Have unrecognized',' input on the line',/7x,'beginning with "',a,'".',' This line should contain the',/7x,'name of a',' component species and corresponding mole fraction',/7x,'for solid solution ',a,', else it should be a',' separator line terminating a sequence of such lines.')
                end if
            end if
        end do
...
1500 format(/' * Error - (XCON3/rd3d8b) Have too many nxmod',/7x,'alter/suppress options. The code is only dimensioned',/7x,'for ',i3,' such options. Reduce the number of such',/7x,'options or increase the dimensioning parameter nxmdpa.')
...
1510 format(/" * Error - (XCON3/rd3d8b) Don't recognize the",' alter/suppress option',/7x,'string "',a,'". This should',' be one of the strings',/7x,'defined in the ukxm array.',' The valid strings are:',/)
...
1600 format(/' * Error - (XCON3/rd3d8b) The iopt option switch',' title line',/7x,'"',a,'"',/7x,'read from the'," input file isn't in the required format.")
...
1610 format(/' * Error - (XCON3/rd3d8b) The iopt option switch',' title line',/7x,'"',a,'"',/7x,'read from the',' input file references an option switch index',/7x,'of ',i3,', which is out of range.')
...
1612 format(/' * Error - (XCON3/rd3d8b) The iopt option switch',' title string',/7x,'"',a,'"',/7x,"read from the input file doesn't contain the",' matching defined string',/7x,'"',a,'".')
...
1630 format(/' * Error - (XCON3/rd3d8b) Have too many option',/7x,'choice lines for the iopt option switch whose title',' string is',/7x,'"',a,'".',/7x,' The code is only',' dimensioned for ',i3,' such lines. Reduce the',/7x,'number of such lines or increase the dimensioning',' parameter jptxpa.')
...
1640 format(/' * Error - (XCON3/rd3d8b) The following iopt',/7x,'option switch choice line read from the input file',/7x,"isn't in the required format:",/7x,'"',a,'"')
...
1650 format(/' * Error - (XCON3/rd3d8b) The iopt option switch',/7x,'choice line',/7x,'"',a,'"',/7x,'read from the input file references an out-of-range',' option',/7x,'choice index.')
...
1660 format(/' * Error - (XCON3/rd3d8b) The iopt option switch',' choice line',/7x,'"',a,'"',/7x," read from the input file doesn't contain the",' matching defined string',/7x,'"',a,'".',/7x,'This line belongs to the option switch whose title',' string is',/7x,'"',a,'".')
...
1670 format(/" * Error - (XCON3/rd3d8b) Don't recognize the",' string "',a,'"',/7x,'that appears on the iopt',' option switch choice line',/7x,'"',a,'"',/7x,'read from the input file. An option choice should',' be chosen by',/7x,'placing a "*", "x", or "X" in the',' checkbox ("[ ]"). This',/7x,'choice line belongs to',' the option switch whose title string is',/7x,'"',a,'".')
...
1680 format(/' * Warning - (XCON3/rd3d8b) No option choice was',' checked on the input file',/7x,'for the iopt option',' switch whose title string is',/7x,'"',a,'".')
...
1694 format(/' * Warning - (XCON3/rd3d8b) More than one option',' choice was checked',/7x,'on the input file for the iopt',' option switch whose title string is',/7x,'"',a,'".',/7x,'The last choice checked will be used. The',' matching string is',/7x,'"',a,'".')
        end if
    end do
...
1700 format(/' * Error - (XCON3/rd3d8b) The iopg option switch',' title line',/7x,'"',a,'"',/7x,'read from the'," input file isn't in the required format.")
...
1712 format(/' * Error - (XCON3/rd3d8b) The iopg option switch',' title line',/7x,'"',a,'"',/7x,'read from the',' input file references an option switch index',/7x,'of ',i3,', which is out of range.')
...
1720 format(/' * Error - (XCON3/rd3d8b) The iopg option switch',' title string',/7x,'"',a,'"',/7x,"read from the input file doesn't contain the",' matching defined string',/7x,'"',a,'".')
...
1730 format(/' * Error - (XCON3/rd3d8b) Have too many option',/7x,'choice lines for the iopg option switch whose title',' string is',/7x,'"',a,'".',/7x,' The code is only',' dimensioned for ',i3,' such lines. Reduce the',/7x,'number of such lines or increase the dimensioning',' parameter jpgxpa.')
...
1740 format(/' * Error - (XCON3/rd3d8b) The following iopg',/7x,'option switch choice line read from the input file',/7x,"isn't in the required format:",/7x,'"',a,'"')
...
1750 format(/' * Error - (XCON3/rd3d8b) The iopg option switch',/7x,'choice line',/7x,'"',a,'"',/7x,'read from the input file references an out-of-range',' option',/7x,'choice index.')
...
1760 format(/' * Error - (XCON3/rd3d8b) The iopg option switch',' choice line',/7x,'"',a,'"',/7x," read from the input file doesn't contain the",' matching defined string',/7x,'"',a,'".',/7x,'This line belongs to the option switch whose title',' string is',/7x,'"',a,'".')
...
1770 format(/" * Error - (XCON3/rd3d8b) Don't recognize the",' string "',a,'"',/7x,'that appears on the iopg',' option switch choice line',/7x,'"',a,'"',/7x,'read from the input file. An option choice should',' be chosen by',/7x,'placing a "*", "x", or "X" in the',' checkbox ("[ ]"). This',/7x,'choice line belongs to',' the option switch whose title string is',/7x,'"',a,'".')
...
1780 format(/' * Warning - (XCON3/rd3d8b) No option choice was',' checked on the input file',/7x,'for the iopg option',' switch whose title string is',/7x,'"',a,'".')
...
1794 format(/' * Warning - (XCON3/rd3d8b) More than one option',' choice was checked',/7x,'on the input file for the iopg',' option switch whose title string is',/7x,'"',a,'".',/7x,'The last choice checked will be used. The',' matching string is',/7x,'"',a,'".')
        end if
    end do
...
1800 format(/' * Error - (XCON3/rd3d8b) The iopr option switch',' title line',/7x,'"',a,'"',/7x,'read from the'," input file isn't in the required format.")
...
1810 format(/' * Error - (XCON3/rd3d8b) The iopr option switch',' title line',/7x,'"',a,'"',/7x,'read from the',' input file references an option switch index',/7x,'of ',i3,', which is out of range.')
...
1812 format(/' * Error - (XCON3/rd3d8b) The iopr option switch',' title string',/7x,'"',a,'"',/7x,"read from the input file doesn't contain the",' matching defined string',/7x,'"',a,'".')
...
1830 format(/' * Error - (XCON3/rd3d8b) Have too many option',/7x,'choice lines for the iopr option switch whose title',' string is',/7x,'"',a,'".',/7x,' The code is only',' dimensioned for ',i3,' such lines. Reduce the',/7x,'number of such lines or increase the dimensioning',' parameter jprxpa.')
...
1840 format(/' * Error - (XCON3/rd3d8b) The following iopr',/7x,'option switch choice line read from the input file',/7x,"isn't in the required format:",/7x,'"',a,'"')
...
1850 format(/' * Error - (XCON3/rd3d8b) The iopr option switch',/7x,'choice line',/7x,'"',a,'"',/7x,'read from the input file references an out-of-range',' option',/7x,'choice index.')
...
1860 format(/' * Error - (XCON3/rd3d8b) The iopr option switch',' choice line',/7x,'"',a,'"',/7x," read from the input file doesn't contain the",' matching defined string',/7x,'"',a,'".',/7x,'This line belongs to the option switch whose title',' string is',/7x,'"',a,'".')
...
1870 format(/" * Error - (XCON3/rd3d8b) Don't recognize the",' string "',a,'"',/7x,'that appears on the iopr',' option switch choice line',/7x,'"',a,'"',/7x,'read from the input file. An option choice should',' be chosen by',/7x,'placing a "*", "x", or "X" in the',' checkbox ("[ ]"). This',/7x,'choice line belongs to',' the option switch whose title string is',/7x,'"',a,'".')
...
1880 format(/' * Warning - (XCON3/rd3d8b) No option choice was',' checked on the input file',/7x,'for the iopr option',' switch whose title string is',/7x,'"',a,'".')
...
1894 format(/' * Warning - (XCON3/rd3d8b) More than one option',' choice was checked',/7x,'on the input file for the iopr',' option switch whose title string is',/7x,'"',a,'".',/7x,'The last choice checked will be used. The',' matching string is',/7x,'"',a,'".')
        end if
    end do
...
1900 format(/' * Error - (XCON3/rd3d8b) The iodb option switch',' title line',/7x,'"',a,'"',/7x,'read from the'," input file isn't in the required format.")
...
1910 format(/' * Error - (XCON3/rd3d8b) The iodb option switch',' title line',/7x,'"',a,'"',/7x,'read from the',' input file references an option switch index',/7x,'of ',i3,', which is out of range.')
...
1920 format(/' * Error - (XCON3/rd3d8b) The iodb option switch',' title string',/7x,'"',a,'"',/7x,"read from the input file doesn't contain the",' matching defined string',/7x,'"',a,'".')
...
1922 format(/' * Error - (XCON3/rd3d8b) Have too many option',/7x,'choice lines for the iodb option switch whose title',' string is',/7x,'"',a,'".',/7x,' The code is only',' dimensioned for ',i3,' such lines. Reduce the',/7x,'number of such lines or increase the dimensioning',' parameter jdbxpa.')
...
1940 format(/' * Error - (XCON3/rd3d8b) The following iodb',/7x,'option switch choice line read from the input file',/7x,"isn't in the required format:",/7x,'"',a,'"')
...
1950 format(/' * Error - (XCON3/rd3d8b) The iodb option switch',/7x,'choice line',/7x,'"',a,'"',/7x,'read from the input file references an out-of-range',' option',/7x,'choice index.')
...
1960 format(/' * Error - (XCON3/rd3d8b) The iodb option switch',' choice line',/7x,'"',a,'"',/7x," read from the input file doesn't contain the",' matching defined string',/7x,'"',a,'".',/7x,'This line belongs to the option switch whose title',' string is',/7x,'"',a,'".')
...
1970 format(/" * Error - (XCON3/rd3d8b) Don't recognize the",' string "',a,'"',/7x,'that appears on the iodb',' option switch choice line',/7x,'"',a,'"',/7x,'read from the input file. An option choice should',' be chosen by',/7x,'placing a "*", "x", or "X" in the',' checkbox ("[ ]"). This',/7x,'choice line belongs to',' the option switch whose title string is',/7x,'"',a,'".')
...
1980 format(/' * Warning - (XCON3/rd3d8b) No option choice was',' checked on the input file',/7x,'for the iodb option',' switch whose title string is',/7x,'"',a,'".')
...
1994 format(/' * Warning - (XCON3/rd3d8b) More than one option',' choice was checked',/7x,'on the input file for the iodb',' option switch whose title string is',/7x,'"',a,'".',/7x,'The last choice checked will be used. The',' matching string is',/7x,'"',a,'".')
        end if
    end do
