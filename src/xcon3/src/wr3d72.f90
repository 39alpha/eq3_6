subroutine wr3d72(cspb,fep,iktmax,iodb,iopg,iopr,iopt,itermx,jflagb,jxmod,kxmod,ncompb,newin,nodbmx,nopgmx,noprmx,noptmx,nsq,nsqmax,ntitl,ntitmx,nttyo,nxmdmx,nxmod,nxtb,nxtmax,rho,tempc,tdspkg,tdspl,tolbt,toldl,tolsat,ubasis,uebal,umemb,uphas1,uphas2,uredox,usolb,uspecb,utitl,xbarb,uxmd24,xlkmod)
...
1080 format('|Electrical Balancing on |',a24,'|',a1,'code selects|',a1,'not performed|')

        write (newin,1095)

        ! Basis species and associated constraints.
        write (newin,1110)
1110 format('| SPECIES',16x,'| BASIS SWITCH/CONSTRAINT|',' CONC/ETC  | UNITS OR TYPE  |')
...
117 continue
            ...
            if (iopt(1).eq.1 .or. iopt(1).eq.-3) then
                write (newin,1140)
1140 format('|redox',t26,'| Defined by next species',t51,'|',t63,'|Redox couple',t80,'|')
...
1145 format(" * Error - (XCON3/wr3d72) Can't find the redox",/7x,'couple-defining  species "',a,'" in the set of',/7x,'basis species.')
...
1160 format(" * Error - (XCON3/wr3d72) Can't find the solid",/7x,'solution "',a,'" in the set of solid solutions',/7x,'for which compositions are defined.')
...
1815 format('|',t26,'|',a24,'|',g11.5,'|Mole fraction',t80,'|')
