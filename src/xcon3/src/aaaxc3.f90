subroutine aaaxc3(ustxc3,uvexc3)
    ...
    !! This is a utility program to convert EQ3NR input files from
    !! one version level to another and from one format to another.
    !! This program is most conveniently run using the XCIF3 utility
    ...
    !!   'W' = compact
    !!   'D' = menu-style
    !! 'D' format does not exist for version level '6.0'.
    !! To specify the choice of version level and format of the new
    !! input file, use the control file IXCON. The same control file
    !! also governs the EQ6 input file converter XCON6. A copy of the
    !! contents of this file can be found in the comments in
    !! EQLIBU/rddixc.f.
    ...
    !! As noted above, it is more convenient to run XCON3 using the
    !! interface software XCIF3 than to run XCON3 directly. On a UNIX
    !! system, XCIF3 is a symbolic link to the shell script xcif36RXX,
    !! where RXX is a revision number (R01, R02, R03, etc.). On a
    !! PC system, XCIF3 is a Fortran program (XCIF3.EXE; the source
    !! code file is XCIF3.FOR) containing DOS system calls. XCIF3
    !! replaces the specified input files. XCIF3 also creates the
    !! IXCON control file used by XCON3, and deletes it prior to
    !! completing execution. To convert a set of EQ3NR input files from
    !! version level 7.2 ('W' or 'D' format) to version level 8.0,
    !! 'W' format, enter:
    !!   xcif3 7.2 8.0 W *.3i
    !! To convert the same files to 'D' format instead, enter:
    !!   xcif3 7.2 8.0 D *.3i
    !! XCON6 and XCIF6 are analogues of XCON3 and XCIF3, respectively.
    !! They perform the analogous conversions of EQ6 input files.
    ...
    !! concatenated source code for XCON3. Concatenation is normally
    ...
    !!   None
    !! Principal output:
    !!   ustxc3 = XCON3 stage number
    !!   uvexc3 = XCON3 version number
    implicit none
