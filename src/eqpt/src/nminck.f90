subroutine nminck(nerr,nmt,nmtmax,noutpt,nttyo,uminsp)
    ...
    !!   EQPT/eqpt.f
    !! Principal input:
    !!   uminsp = array containing the names of pure minerals
    !! Principal output:
...
100 continue
        ...
        if (ncount .gt. 1) then
            ...
            if (ilist .eq. 1) then
                ...
                write (noutpt,1000)
                write (nttyo,1000)
1000 format(/' * Error - (EQPT/nminck) The following pure',' minerals have multiple',/7x,'species blocks:',/)
            end if
