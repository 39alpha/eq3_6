subroutine rdppsi(apsi,jpfcmx,nat,natmax,ndat0s,nerr,noutpt,npxpsi,npx3mx,npx3t,nttyo,nwarn,uaqsp,utripl,zaqsp)
    ...
    !! anion-different anion-cation (aa'c) triplets. This particular
    !! set of interaction coefficients includes the following:
    !!   1. psi(cc'a) and psi(aa'c)
    ...
    !!   npxpsi = the number of cc'a and aa'c triplets for which Pitzer
...
1000 format(a80)
    ...
    if (uline(1:24) .ne. uhdpsi(1:24)) then
        go to 100
    end if
...
1020 format(/' * Error - (EQPT/rdppsi) Have encountered a blank',' species name',/7x,'in the species triplet for block ',a,' of the superblock for',/7x,"Pitzer cc'a and aa'c parameter",' data. The offending triplet shows as',/7x,a,', ',a,', ',a,'.')
...
1100 format(/' * Error - (EQPT/rdppsi) The aqueous species ',a,' appearing in',/7x,"the Pitzer cc'a and aa'c parameter",' data block for the species triplet',/7x,a,', ',a,', ',a,' does not appear in an',/7x,'aqueous species data block.')
...
1200 format(/' * Error - (EQPT/rdppsi) Have found an illegal data',' block for the',/7x,'species triplet ',a,', ',a,', ',a,' in the superblock',/7x,"of Pitzer data for cc'a and aa'c",' triplets. Solvent water may not',/7x,'appear in such a',' triplet.')
    ...
    if (n1.ne.0 .and. n2.ne.0 .and. n3.ne.0) then
        ...
        ! current superblock. It must be of type cc'a or aa'c. Other
        ! triplet types such as nca, nnn, nnn', and nn'n' are not
        ! permitted here.
...
1220 format(/' * Error - (EQPT/rdppsi Have found an',' illegal data block for the',/7x,'species triplet ',a,', ',a,', ',a,' in the',/7x,'superblock of Pitzer'," data for cc'a and aa'c triplets.",/7x,'The present',' triplet type is not valid in the Pitzer framework.')

            nerr = nerr + 1
        else if (.not.(nc.eq.2 .and. na.eq.1)    .and. .not.(na.eq.2 .and. nc.eq.1)) then
...
1230 format(/' * Error - (EQPT/rdppsi Have found an',' illegal data block for the',/7x,'species triplet ',a,', ',a,', ',a,' in the',/7x,'superblock of Pitzer'," data cc'a and aa'c triplets.",/7x,'The present',' triplet type may not appear in the current superblock.')
    ...
    ! Here: the odd ion goes last. The alphabetical rule applies for
    ! the other two.
    ...
    ! Read the coefficients for the psi(cc'a) or psi(aa'c)
    ! parameters.
...
1330 format(/' * Error - (EQPT/rdppsi) Have found a line',' starting with:',/7x,'"',a,'"',/7x,'where "',a,'" was',' expected in the block for the species',/7x,'triplet ',a,', ',a,', ',a,' in the superblock of Pitzer data',/7x,"for cc'a and aa'c triplets.")
...
1320 format(/' * Error - (EQPT/rdppsi) Have found a line',' starting with:',/7x,'"',a,'"',/7x,'containing an',' expected numerical field that could not be read.',/7x,'This occurred in the block for the species triplet',/7x,a,', ',a,', ',a,' in the superblock of Pitzer data',/7x,"for cc'a and aa'c triplets.")
...
1470 format(/' * Error - (EQPT/rdppsi) Have found a line starting',' with:',/7x,'"',a,'"',/7x,' in the data block for the species',' triplet ',a,', ',a,', ',a,/7x,'in the superblock of'," Pitzer data for cc'a and aa'c triplets. Should have",/7x,'found the delimiter line marking the end of the block.')
...
990 continue
    write (noutpt,2010)
    write (nttyo,2010)
2010 format(/' * Error - (EQPT/rdppsi) Unexpectedly encountered',/7x,"an end-of-file error while reading the cc'a and aa'c",/7x,'(psi) superblock of the DATA0 file.')
...
995 continue
    write (noutpt,2020)
    write (nttyo,2020)
2020 format(/' * Error - (EQPT/rdppsi) Encountered a read format',/7x,"error while reading the cc'a and aa'c (psi) superblock",/7x,'of the DATA0 file.')

    stop

999 continue
end subroutine rdppsi