subroutine rdpnn(abeta,ipbtmx,jpfcmx,nat,natmax,ndat0s,nerr,noutpt,npxnn,npx2mx,npx2t,nttyo,nwarn,uaqsp,upair,zaqsp)
    ...
    !! following:
    !!   1. lambda(nn')
    !! Although mu(nnn') and mu(nn'n') are closely related to
    !! lambda(nn'), they are not read from the current superblock
    !! by this subroutine, but from a triplets superblock read by
    !! another subroutine, rdpn2n.f. There are several reasons for
    !! this treatment. First, the relevant type of mu here is more
    !! analogous to a psi parameter than a Cphi parameter. Note that
    !! mu(nnn) is included with lambda(nn) in part because mu(nnn)
    !! is analogous to a Cphi parameter. Second, data for the current
    !! type of mu is both scarce and not very likely to be needed
    ...
    !!   npxnn  = the number of nn pairs for which Pitzer parameters
...
1000 format(a80)
    ...
    if (uline(1:24) .ne. uhdnn(1:24)) then
        go to 100
    end if
...
1020 format(/' * Error - (EQPT/rdpnn) Have encountered a blank',' species name',/7x,'in the species pair for block ',a,' of the superblock for',/7x,"Pitzer nn' parameter data.",' The offending pair shows as ',a,', ',a,'.')
...
1100 format(/' * Error - (EQPT/rdpnn) The aqueous species ',a,' appearing in',/7x,"the Pitzer nn' parameter data block",' for the species pair',/7x,a,', ',a,' does not appear',' in an aqueous species',/7x,'data block.')
...
1200 format(/' * Error - (EQPT/rdpnn) Have found an illegal',' data block for the',/7x,'species pair ',a,', ',a,' in the superblock of Pitzer data',/7x,"for nn' pairs.",' Solvent water may not appear in such a pair.')
    ...
    if (n1.gt.0 .and. n2.gt.0) then
        ...
        ! current superblock. It must be of type nn '. Other
...
1230 format(/' * Error - (EQPT/rdpnn) Have found an',' illegal data block for the',/7x,'species pair ',a,', ',a,' in the',/7x,'superblock of Pitzer data for'," nn' pairs.",/7x,'The present pair type may not',' appear in the current superblock.')
    ...
    if (lle(unam1,unam2)) then
        ! The first species is first alphabetically.
        upair(1,npx2) = unam1
        upair(2,npx2) = unam2
    else
        ! The second species is first alphabetically.
        upair(1,npx2) = unam2
        upair(2,npx2) = unam1
    end if
...
1330 format(/' * Error - (EQPT/rdpnn) Have found a line',' starting with:',/7x,'"',a,'"',/7x,'where "',a,'" was',' expected in the block for the species',/7x,'pair ',a,', ',a,' in the superblock of Pitzer data',/7x,"for nn' pairs.")
...
1320 format(/' * Error - (EQPT/rdpnn) Have found a line',' starting with:',/7x,'"',a,'"',/7x,'containing an',' expected numerical field that could not be read.',/7x,'This occurred in the block for the species pair',/7x,a,', ',a,' in the superblock of Pitzer data',/7x,"for nn' pairs.")
...
1470 format(/' * Error - (EQPT/rdpnn) Have found a line starting',' with:',/7x,'"',a,'"',/7x,' in the data block for the species',' pair ',a,', ',a,' in the',/7x,'superblock of Pitzer data for'," nn' pairs. Should have",/7x,'found the delimiter line',' marking the end of the block.')
...
990 continue
    write (noutpt,2010)
    write (nttyo,2010)
2010 format(/' * Error - (EQPT/rdpnn) Unexpectedly encountered',/7x,"an end-of-file error while reading the nn' (lambda)",/7x,'superblock of the DATA0 file.')
...
995 continue
    write (noutpt,2020)
    write (nttyo,2020)
2020 format(/' * Error - (EQPT/rdpnn) Encountered a read format',/7x,"error while reading the nn' (lambda) superblock of",/7x,'the DATA0 file.')

    stop

999 continue
end subroutine rdpnn