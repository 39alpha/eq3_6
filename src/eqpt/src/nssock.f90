subroutine nssock(nerr,noutpt,nttyo,nxt,nxtmax,ussoph)
    ...
    ! Note: ilist = the number of solid solutions with duplicate
    ! phase blocks present on the data file.
    ilist = 0
...
100 continue
        ...
        if (ncount .gt. 1) then
            ...
            if (ilist .eq. 1) then
                ...
                write (noutpt,1000)
                write (nttyo,1000)
1000 format(/' * Error - (EQPT/nssock) The following solid',' solutions have multiple',/7x,'phase blocks:',/)
            end if
