subroutine rdpn2n(apsi,jpfcmx,nat,natmax,ndat0s,nerr,noutpt,npxn2n,npx3mx,npx3t,nttyo,nwarn,uaqsp,utripl,zaqsp)
    ...
    !! triplets. This particular set of interaction coefficients
    !! includes the following:
    !!   1. mu(nnn')
    !! Note that to have both mu(nnn') and mu(n'n'n), there must
    !! be a data block for each of the nnn' and n'n'n combinations.
    ...
    !!   npxn2n = the number of nnn' triplets for which Pitzer
...
1000 format(a80)
    ...
    if (uline(1:24) .ne. uhdn2n(1:24)) then
        go to 100
    end if
...
1020 format(/' * Error - (EQPT/rdpn2n) Have encountered a blank',' species name',/7x,'in the species triplet for block ',a,' of the superblock for',/7x,"Pitzer nnn' parameter data.",' The offending triplet shows as',/7x,a,', ',a,', ',a,'.')
...
1100 format(/' * Error - (EQPT/rdpn2n) The aqueous species ',a,' appearing in',/7x,"the Pitzer nnn' parameter data",' block for the species triplet',/7x,a,', ',a,', ',a,' does not appear in an',/7x,'aqueous species data block.')
...
1200 format(/' * Error - (EQPT/rdpn2n) Have found an illegal data',' block for the',/7x,'species triplet ',a,', ',a,', ',a,' in the superblock',/7x,"of Pitzer data for nnn' triplets.",' Solvent water may not',/7x,'appear in such a triplet.')
    ...
    if (n1.ne.0 .and. n2.ne.0 .and. n3.ne.0) then
        ...
        ! current superblock. It must be of type nnn'. Other triplet
        ...
        qdup1p = (qdup12 .and. .not.qdup23 .and. .not.qdup13)  .or. (qdup23 .and. .not.qdup12 .and. .not.qdup13)  .or. (qdup13 .and. .not.qdup23 .and. .not.qdup12)
...
1220 format(/' * Error - (EQPT/rdpn2n Have found an',' illegal data block for the',/7x,'species triplet ',a,', ',a,', ',a,' in the',/7x,'superblock of Pitzer'," data for nnn' triplets.",/7x,'The present triplet',' type is not valid in the Pitzer framework.')

            nerr = nerr + 1
        else if (.not.(nn.eq.3 .and. qdup1p)) then
...
1230 format(/' * Error - (EQPT/rdpn2n Have found an',' illegal data block for the',/7x,'species triplet ',a,', ',a,', ',a,' in the',/7x,'superblock of Pitzer'," data nnn' triplets. The present",/7x,'triplet type',' may not appear in the current superblock.')
    ...
    ! Read the coefficients for the mu(nnn') parameters. Store the
    ! mu data as psi data.
...
1330 format(/' * Error - (EQPT/rdpn2n) Have found a line',' starting with:',/7x,'"',a,'"',/7x,'where "',a,'" was',' expected in the block for the species',/7x,'triplet ',a,', ',a,', ',a,' in the superblock of Pitzer data',/7x,"for nnn' triplets.")
...
1320 format(/' * Error - (EQPT/rdpn2n) Have found a line',' starting with:',/7x,'"',a,'"',/7x,'containing an',' expected numerical field that could not be read.',/7x,'This occurred in the block for the species triplet',/7x,a,', ',a,', ',a,' in the superblock of Pitzer data',/7x,"for nnn' triplets.")
...
1470 format(/' * Error - (EQPT/rdpn2n) Have found a line starting',' with:',/7x,'"',a,'"',/7x,' in the data block for the species',' triplet ',a,', ',a,', ',a,/7x,'in the superblock of'," Pitzer data for nnn' triplets. Should have",/7x,'found the delimiter line marking the end of the block.')
...
990 continue
    write (noutpt,2010)
    write (nttyo,2010)
2010 format(/' * Error - (EQPT/rdpn2n) Unexpectedly encountered',/7x,"an end-of-file error while reading the nnn' (mu)",/7x,'superblock of the DATA0 file.')
...
995 continue
    write (noutpt,2020)
    write (nttyo,2020)
2020 format(/' * Error - (EQPT/rdpn2n) Encountered a read format',/7x,"error while reading the nnn' (mu) superblock of",/7x,'the DATA0 file.')

    stop

999 continue
end subroutine rdpn2n