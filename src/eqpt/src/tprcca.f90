subroutine tprcca(amucca, amuc2a, apsi, icapr, iccatr, jpfcmx, natmax, ncapr, nccatr, ncvcca, nc2atr, nerr, noutpt, npx3mx, npx3t, nttyo, nwarn, pcvcca, qpdca, qpdcca, uaqsp, utripl, zaqsp)
    ...
    !! the same cc'a triplet). Calculate the conventional primitive
    !! Pitzer parameters from the observable compound parameters read
    !! from the data file:
    !!   psi(cc'a) -> mu(cc'a)
    !! Note: mu(cc'a) depends on cphi(ca) and cphi(c'a) as well as
    !! psi(cc'a).
    !! Check the coverage of entered Pitzer data against all possible
    !! cc'a triplets that can be composed of the cations and anions
    !! present on the data file.
    !! This subroutine is complementary to tpraac.f, which does the
    !! same thing for aa'c (anion, different anion, cation) triplets.
    ...
    ! Check the entered data for cc'a triplets.
    nodatc = 0
    ...
    do n = 1,nccatr
        ...
        if (jtripl .le. 0) then
            ...
            ! Note that qpdcca(n) is left with a value of .false.
            nodatc = nodatc + 1
        else
            ! Have found an entry.
            qpdcca(n) = .true.
...
1010 format(/' * Error - (EQPT/tprcca) Have found a',' duplicate data block',/7x,'on the DATA0 file'," for the cc'a triplet",/7x,a,', ',a,', ',a,'.')
                else
                    ux8 = ' '
                    write (ux8,'(i5)') ndupl
                    call lejust(ux8)
...
1020 format(/' * Error - (EQPT/tprcca) Have found ',a,' duplicate data blocks',/7x,'on the DATA0 file'," for the cc'a triplet",/7x,a,', ',a,', ',a,'.')
                end if
...
140 continue
            ...
            if (qfound1 .and. qfound2) then
                do j = 1,jpfcmx
                    amucca(j,n) = (apsi(j,jtripl) +        3.0*((z2/z1)*amu1(j) + (z1/z2)*amu2(j)))/6.
                end do
            else
...
1030 format(/' * Error - (EQPT/tprcca) Have data on the DATA0'," file for the cc'a",/7x,'triplet ',a,', ',a,', ',a,', but'," don't have the",/7x,'required data for the following',' cation-anion pair(s):',/)
...
1040 format(9x,a,', ',a)
    ...
    if (nodatc .gt. 0) then
        write (noutpt,1060)
        write (nttyo,1060)
1060 format(/' * Warning - (EQPT/tprcca) Did not find a data',' block on the DATA0 file',/7x,'for any of the following'," cc'a triplets:",/)
