subroutine rdpn2(abeta,acphi,ipbtmx,jpfcmx,nat,natmax,ndat0s,nerr,noutpt,npxn2,npx2mx,npx2t,nttyo,nwarn,uaqsp,upair,zaqsp)
    !! This subroutine reads from the DATA1 file the coefficients
    !! required to compute those Pitzer interaction parameters
    !! associated with neutral-same neutral pairs (the pair
    !! business is really a formalism maintained for consistency
    !! with other parameter sets , as just the neutral n itself
    !! is sufficient to identify the set). This particular
    ...
    !!   npxn2  = the number of nn pairs for which Pitzer parameters
...
1000 format(a80)
    ...
    if (uline(1:24) .ne. uhdn2(1:24)) then
        go to 100
    end if
...
1010 format(a24,2x,a24)
    ...
    ! Note that for nn pairs, the species name need not be repeated.
    ! If unam2 is blank, it is merely set to unam1.
    j2 = ilnobl(unam1)
...
1020 format(/' * Error - (EQPT/rdpn2) Have encountered a blank',' species name',/7x,'in the species pair for block ',a,' of the superblock for',/7x,'Pitzer nn parameter data.',' The offending pair shows as ',a,', ',a,'.')
...
1100 format(/' * Error - (EQPT/rdpn2) The aqueous species ',a,' appearing in',/7x,'the Pitzer nn parameter data block',' for the species pair',/7x,a,', ',a,' does not appear',' in an aqueous species',/7x,'data block.')
...
1200 format(/' * Error - (EQPT/rdpn2) Have found an illegal',' data block for the',/7x,'species pair ',a,', ',a,' in the superblock of Pitzer data',/7x,"for nn pairs.",' Solvent water may not appear in such a pair.')
    ...
    if (n1.gt.0 .and. n2.gt.0) then
        ...
        ! current superblock. It must be of type nn. Other pair
        ! pair types such as ca, cc', aa', nc, na, and nn' are not
        ! permitted here.
...
1230 format(/' * Error - (EQPT/rdpn2) Have found an',' illegal data block for the',/7x,'species pair ',a,', ',a,' in the',/7x,'superblock of Pitzer data for'," nn pairs.",/7x,'The present pair type',' may not appear in the current superblock.')
    ...
    ! Here: just one species in the pair.
    upair(1,npx2) = unam1
    upair(2,npx2) = unam1

    ! Read the coefficients for the lambda(nn) parameters.
    ! Store the lambda data as beta(0) data.
...
1330 format(/' * Error - (EQPT/rdpn2) Have found a line',' starting with:',/7x,'"',a,'"',/7x,'where "',a,'" was',' expected in the block for the species',/7x,'pair ',a,', ',a,' in the superblock of Pitzer data',/7x,"for nn pairs.")
...
1320 format(/' * Error - (EQPT/rdpn2) Have found a line',' starting with:',/7x,'"',a,'"',/7x,'containing an',' expected numerical field that could not be read.',/7x,'This occurred in the block for the species pair',/7x,a,', ',a,' in the superblock of Pitzer data',/7x,"for nn pairs.")
    ...
    ! Now read the mu data. Store the mu data as Cphi data.
    ! Check the required parameter header.
    read (ndat0s,1000,end=990,err=995) uline
    j4 = ilnobl(uline)
    j4 = min(j4,70)
...
1470 format(/' * Error - (EQPT/rdpn2) Have found a line starting',' with:',/7x,'"',a,'"',/7x,' in the data block for the species',' pair ',a,', ',a,' in the',/7x,'superblock of Pitzer data for'," nn pairs. Should have",/7x,'found the delimiter line',' marking the end of the block.')
...
990 continue
    write (noutpt,2010)
    write (nttyo,2010)
2010 format(/' * Error - (EQPT/rdpn2) Unexpectedly encountered',/7x,"an end-of-file error while reading the nn (lambda, mu)",/7x,'superblock of the DATA0 file.')
...
995 continue
    write (noutpt,2020)
    write (nttyo,2020)
2020 format(/' * Error - (EQPT/rdpn2) Encountered a read format',/7x,"error while reading the nn (lambda, mu) superblock",/7x,'of the DATA0 file.')

    stop

999 continue
end subroutine rdpn2