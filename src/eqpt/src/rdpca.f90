subroutine rdpca(abeta,alpha,acphi,ipbtmx,jpfcmx,nat,natmax,ndat0s,nerr,noutpt,npxca,npx2mx,npx2t,nttyo,nwarn,uaqsp,upair,zaqsp)
    ...
    !! set of interaction coefficients includes the following:
    !!   1. beta(0)(ca), beta(1),(ca) and beta(2)(ca)
    !!   2. Cphi(ca)
    !! The alpha(1) and alpha(2) parameters which go with the beta
    !! parameters are also read to allow increased flexibility over
    !! simple assignment according to ca-pair type.
    ...
    !!   npxca  = the number of ca pairs for which Pitzer parameters
...
1000 format(a80)
    ...
    if (uline(1:24) .ne. uhdca(1:24)) then
        go to 100
    end if
...
1020 format(/' * Error - (EQPT/rdpca) Have encountered a blank',' species name',/7x,'in the species pair for block ',a,' of the superblock for',/7x,'Pitzer ca parameter data.',' The offending pair shows as ',a,', ',a,'.')
...
1100 format(/' * Error - (EQPT/rdpca) The aqueous species ',a,' appearing in',/7x,'the Pitzer ca parameter data block',' for the species pair',/7x,a,', ',a,' does not appear',' in an aqueous species',/7x,'data block.')
...
1200 format(/' * Error - (EQPT/rdpca) Have found an illegal',' data block for the',/7x,'species pair ',a,', ',a,' in the superblock of Pitzer data',/7x,'for ca pairs.',' Solvent water may not appear in such a pair.')
    ...
    if (n1.gt.0 .and. n2.gt.0) then
        ...
        ! current superblock. It must be of type ca. Other pair types
        ! such as cc', aa', nc, na, nn, and nn' are not permitted here.
...
1230 format(/' * Error - (EQPT/rdpca) Have found an',' illegal data block for the',/7x,'species pair ',a,', ',a,' in the',/7x,'superblock of Pitzer data for',' ca pairs. The present pair type',/7x,'may not appear',' in the current superblock.')
    ...
    ! Here: the cation comes first.
    if (iz1 .gt. 0) then
        ! The first species is the cation.
        upair(1,npx2) = unam1
        upair(2,npx2) = unam2
    else
        ! The second species is the cation.
        upair(1,npx2) = unam2
        upair(2,npx2) = unam1
    end if

    ! Read the coefficients for the beta and Cphi parameters.
    ! First peek ahead to see if alpha parameters are also specified
...
1310 format(/' * Error - (EQPT/rdpca) Have found a line',' starting with:',/7x,'"',a,'"',/7x,'where an "alpha" or',' "beta" header was expected in the block for',/7x,'the',' species pair ',a,', ',a,' in the',/7x,'superblock of',' Pitzer data for ca pairs.')
    ...
    if (ux80(1:4) .eq. 'beta') then
        ...
        ! Note that a full set of alpha parameters must be specified
        ! if any alpha data are present.
        do i = 1,ipbtmx
            read (ndat0s,1000,end=990,err=995) uline
            j4 = ilnobl(uline)
            j4 = min(j4,70)
...
1320 format(/' * Error - (EQPT/rdpca) Have found a line',' starting with:',/7x,'"',a,'"',/7x,'containing an',' expected numerical field that could not be read.',/7x,'This occurred in the block for the species pair',/7x,a,', ',a,' in the superblock of Pitzer data',/7x,'for ca pairs.')
...
1330 format(/' * Error - (EQPT/rdpca) Have found a line',' starting with:',/7x,'"',a,'"',/7x,'where "',a,'" was',' expected in the block for the species',/7x,'pair ',a,', ',a,' in the superblock of Pitzer data',/7x,'for ca pairs.')
...
1470 format(/' * Error - (EQPT/rdpca) Have found a line starting',' with:',/7x,'"',a,'"',/7x,' in the data block for the species',' pair ',a,', ',a,' in the',/7x,'superblock of Pitzer data for',' ca pairs. Should have found the',/7x,'delimiter line',' marking the end of the block.')
...
990 continue
    write (noutpt,2010)
    write (nttyo,2010)
2010 format(/' * Error - (EQPT/rdpca) Unexpectedly encountered',/7x,'an end-of-file error while reading the ca superblock',/7x,'of the DATA0 file.')
...
995 continue
    write (noutpt,2020)
    write (nttyo,2020)
2020 format(/' * Error - (EQPT/rdpca) Encountered a read format',/7x,'error while reading the ca superblock of the',/7x,'DATA0 file.')

    stop

999 continue
end subroutine rdpca