subroutine rdpth(abeta, ipbtmx, jpfcmx, nat, natmax, ndat0s, nerr, noutpt, npxth, npx2mx, npx2t, nttyo, uaqsp, upair, zaqsp)
    ...
    !! different anion (aa') pairs. This particular set of interaction
    !! coefficients includes the following:
    !!   1. theta(cc'), theta(aa')
    ...
    !!   npxth  = the number of cc' and aa' pairs for which Pitzer
...
1020 format(/' * Error - (EQPT/rdpth) Have encountered a blank',' species name',/7x,'in the species pair for block ',a,' of the superblock for',/7x,"Pitzer cc' and aa' parameter",' data. The offending pair shows as ',a,', ',a,'.')
...
1100 format(/' * Error - (EQPT/rdpth) The aqueous species ',a,' appearing in',/7x,"the Pitzer cc' and aa' parameter data",' block for the species pair',/7x,a,', ',a,' does not appear',' in an aqueous species',/7x,'data block.')
...
1200 format(/' * Error - (EQPT/rdpth) Have found an illegal',' data block for the',/7x,'species pair ',a,', ',a,' in the superblock of Pitzer data',/7x,"for cc' and aa'",' pairs. Solvent water may not appear in such a pair.')
    ...
    if (n1.gt.0 .and. n2.gt.0) then
        ...
        ! current superblock. It must be of type cc' or aa'. Other
        ! pair types such as ca, nc, na, nn, and nn' are not
        ! permitted here.
        if ((iz1*iz2) .lt. 0) then
            if (n1 .eq. n2) then
                ! The current data block refers to a pair combination
                ! that is not valid in the Pitzer framework.
                write (noutpt,1220) unam1(1:j2),unam2(1:j3)
                write (nttyo,1220) unam1(1:j2),unam2(1:j3)
1220 format(/' * Error - (EQPT/rdpth) Have found an',' illegal data block for the',/7x,'species pair ',a,', ',a,' in the',/7x,'superblock of Pitzer data for'," cc' and aa' pairs.",/7x,'The present pair',' type is not valid in the Pitzer framework.')
            else
...
1230 format(/' * Error - (EQPT/rdpth) Have found an',' illegal data block for the',/7x,'species pair ',a,', ',a,' in the',/7x,'superblock of Pitzer data for'," cc' and aa' pairs.",/7x,'The present pair',' type may not appear in the current superblock.')
            end if
    ...
    if (lle(unam1,unam2)) then
        ! The first species comes first alphabetically.
        upair(1,npx2) = unam1
        upair(2,npx2) = unam2
    else
        ! The second species comes first alphabetically.
        upair(1,npx2) = unam2
        upair(2,npx2) = unam1
    end if

    ! Read the coefficients for the theta(cc') and theta(aa')
    ! parameters. Store the theta data as beta(0) data.
...
1330 format(/' * Error - (EQPT/rdpth) Have found a line',' starting with:',/7x,'"',a,'"',/7x,'where "',a,'" was',' expected in the block for the species',/7x,'pair ',a,', ',a,' in the superblock of Pitzer data',/7x,"for cc' and aa' pairs.")
...
1320 format(/' * Error - (EQPT/rdpth) Have found a line',' starting with:',/7x,'"',a,'"',/7x,'containing an',' expected numerical field that could not be read.',/7x,'This occurred in the block for the species pair',/7x,a,', ',a,' in the superblock of Pitzer data',/7x,"for cc' and aa' pairs.")
...
1470 format(/' * Error - (EQPT/rdpth) Have found a line starting',' with:',/7x,'"',a,'"',/7x,' in the data block for the species',' pair ',a,', ',a,' in the',/7x,'superblock of Pitzer data for'," cc' and aa' pairs. Should have",/7x,'found the',' delimiter line marking the end of the block.')
...
990 continue
    write (noutpt,2010)
    write (nttyo,2010)
2010 format(/' * Error - (EQPT/rdpth) Unexpectedly encountered',/7x,"an end-of-file error while reading the cc' and aa'",/7x,'(theta) superblock of the DATA0 file.')
...
995 continue
    write (noutpt,2020)
    write (nttyo,2020)
2020 format(/' * Error - (EQPT/rdpth) Encountered a read format',/7x,"error while reading the cc' and aa' (theta) superblock",/7x,'of the DATA0 file.')

    stop

999 continue
end subroutine rdpth