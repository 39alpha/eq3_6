subroutine tpraa(alamaa,atheta,iaapr,ipbtmx,jpfcmx,naapr,natmax,ncvaa,nerr,noutpt,npx3mx,nthdt,nttyo,nwarn,pcvaa,qpdaa,uaqsp,uthdtr)
    ...
    !! aa' pair). Calculate the conventional primitive Pitzer parameters
    ...
    !! anion-distinct anion pairs that can be composed of the aqueous
    !! anions present on the data file.
    !! This subroutine is complementary to tprcc.f, which does the
    !! same thing for cc' (cation, different cation) pairs.
    ...
    ! Check the entered data for aa' pairs.
    nodatc = 0
...
1010 format(/' * Error - (EQPT/tpraa) Have found a',' duplicate data block on the DATA0 file',/7x,"for the aa' pair ",a,', ',a,'.')
                else
                    ux8 = ' '
                    write (ux8,'(i5)') ndupl
                    call lejust(ux8)
...
1020 format(/' * Error - (EQPT/tpraa) Have found ',a,' duplicate data blocks on the DATA0 file',/7x,"for the aa' pair ",a,', ',a,'.')
                end if
        ...
        if (jthpr.le.0) then
            ...
            ! Note that qpdaa(n) is left with a value of .false.
            nodatc = nodatc + 1
        end if
    end do
    ...
    if (nodatc .gt. 0) then
        write (noutpt,1040)
        write (nttyo,1040)
1040 format(/' * Warning - (EQPT/tpraa) Did not find a data',' block on the DATA0 file',/7x,'for any of the following'," aa' pairs:",/)
