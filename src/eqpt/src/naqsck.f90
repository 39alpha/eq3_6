subroutine naqsck(nat,natmax,nerr,noutpt,nttyo,uaqsp)
    ...
    !!   EQPT/eqpt.f
    !! Principal input:
    !!   uaqsp  = array containing the names of aqueous species
    !! Principal output:
    ...
    ! Note: ilist = the number of species with duplicate blocks
    ! on the data file.
    ilist = 0
...
100 continue
        ...
        if (ncount .gt. 1) then
            ...
            if (ilist .eq. 1) then
                ...
                write (noutpt,1000)
                write (nttyo,1000)
1000 format(/' * Error - (EQPT/naqsck) The following aqueous',' species have multiple',/7x,'species blocks:',/)
            end if
