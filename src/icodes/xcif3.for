      program xcif3
c
c     XCIF3: XCON3 interface software
...
c-----------------------------------------------------------------------
c
c     This is a PC utility program to run XCON3, the EQ3NR input file
...
c     that script is called as "xcif3".
c
...
c
c     To use this code, the XCON3 executable must be in the code
...
c
      logical qerr,qex
c
      character(len=80) ucodir,ucodrn,ucomln,uline,ulscr,upathn,uxc3c
...
c
      if (nargv .lt. 4) then
        write (nttyo,1000)
 1000   format(' Usage: xcif3 oldversion newversion newformat',
     $  ' inputfile(s)',
...
c
c     Get the XCON3 executable.
c
      uxc3c = ucodir(1:j) // '\xcon3.exe'
c
c     Check the XCON3 executable.
c
      inquire(file=uxc3c,exist=qex)
      if (.not.qex) then
        write (nttyo,1060) ucodir(1:j),ucodrn(1:k)
 1060   format (/" * Error - The executable code xcon3.exe doesn't",
...
c       nferr  = number of input files which are not converted
c                  due to XCON3 errors
c       nfnoex = number of input files which are not converted because
c                  they don't exist
c
      nfrun = 0
      nfwrex = 0
...
c
 ...
 1092       format (/" * Error - Can't run XCIF3 on input file ",'"',a,
     $      '"',
     $      /7x,"because it doesn't have a .3i file name extension.",/)
...
c
          if (qerr) then
            write (nttyo,1130)
 1130       format(/" * Error - XCON3 couldn't write a reformatted",
...
c
 ...
 1160   format(/'  The following input files were not converted due to',
     $  /'  errors encountered while running XCON3:',/)
        do n = 1,nferr
          j2 = ilnobl(uferr(n))
          j2 = min(j2,74)
