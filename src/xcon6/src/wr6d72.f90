subroutine wr6d72(cdac,cesrb,csigma,dlzmx1,dlzmx2,dlzidp,dzprlg,dzprnt,eact,electr,fk,iact,iktbt,iktmax,imchmx,imech,iodb,iopg,iopr,iopt,ioscan,itermx,jcode,jreac,jtemp,jxmod,kct,kdim,kmax,kmt,kprs,ksq,ksppmx,kstpmx,kxmod,kxt,hact,modr,moffg,morr,mprs,mteaqb,mteb,nctmax,ndact,ndctmx,nesrbt,newin,nffg,nffgmx,nmodl1,nmodl2,nodbmx,nopgmx,noprmx,noptmx,nordlm,npslmx,nprmn,nprmx,nprsmx,nrct,nrctmx,nrk,nsk,nsrtmx,nsscmx,nsslmx,ntitl1,ntitl2,ntitmx,ntrymx,nttkmx,nxmdmx,nxmod,nxopex,nxopmx,nxopt,nxpemx,nxrtmx,rk0,rxbarb,sscrew,sk,tempci,tempc0,timemx,tolbt,toldl,tolsat,tolsst,tolx,trk0,tstrt,ttk,ucode,udac,uelemb,uendb,uesrb,ueqlrn,ueqlst,uffg,undms,unrms,uprs,ureac,urelno,ustage,utitl1,utitl2,uxct16,uxmd24,uxopex,uxopt,vreac,xlkffg,xlkmod,zimax,zistrt,zkfac,zklogl,zklogu,zvclgi)
    ...
    character(len=24) :: ux24(3)
    character(len=24) :: ublk24
    character(len=24) :: unone
    character(len=24) :: uxx
    character(len=16) :: udef(nto6pa)
    character(len=16) :: ujx(0:3)
    character(len=16) :: umint(0:4)
    character(len=8) :: usup(-1:2)
    character(len=8) :: ux8(3)
    character(len=8) :: ublk8
...
1120 format('| calculational mode   |',a1,'normal',t37,'|',a1,'economy',t57,'|',a1,'super economy',t72,'|')
        ...
        if (nmodl1 .eq. 1) then
            ux1(1) = '*'
        else if (nmodl1.eq.2 .or. nmodl1.eq.0) then
            ux1(2) = '*'
        else if (nmodl1 .eq. 3) then
            ux1(3) = '*'
        else
            ux1(1) = '*'
        end if
...
1130 format('| model type',t24,'|',a1,'titration',t37,'|',a1,'closed',t57,'|',a1,'open',t72,'|')
...
1140 format('| temperature model',t24,'|',a1,'power',t37,'|',a1,'fluid mixing',t72,'|')

        write (newin,1090)
        write (newin,1150)
1150 format('c power model  -->   temp = tstart + tk1*zi +',' tk2*zi**2 + tk3*zi**3',t72,'|',/'c mixing model -->   temp = (tstart * tk1 + zi*tk2) /',' (zi + tk1)',t72,'|',/'c',70('-'),'|')

        write (newin,1155) tempc0,(ttk(i), i = 1,3)
1155 format('| tstart(c)|',1pg11.4,'|tk1|',1pg11.4,'|tk2|',1pg11.4,'|tk3|',1pg11.4,t72,'|')

        ! Zi and time parameters.
        write (newin,1090)
        write (newin,1160) zistrt,zimax
1160 format('| starting value of zi |',1pg12.5,'|max. value of zi',t58,'|',1pg12.5,t72,'|')

        write (newin,1090)
        write (newin,1170) tstrt,timemx
1170 format('| starting time (sec)  |',1pg12.5,'|max. time (sec)',t58,'|',1pg12.5,t72,'|')

        write (newin,1090)
        write (newin,1175) kstpmx,ksppmx
1175 format('| max. steps',t24,'|',i10,2x,'|max. steps w/o print|',i10,t72,'|')

        ! Note: cplim does not appear on the "D" format input file.
        ! Print interval parameters.
        write (newin,1090)
        write (newin,1180) dzprnt,dzprlg
1180 format('| linear print interval|',1pg12.5,'|log print interval  |',1pg12.5,t72,'|')

        ! Plot interval parameters.
        ! Note: dzplot, dzpllg, and ksplmx do not appear on the "D"
        ! format input file.
...
1200 format('| phases w/ elements| ',a8,t48,'| ',a8,t75,'|n')
...
1210 format('| phases except',t21,'| ',a24,t48,'| ',a24,t75,'|n')
...
150 continue
                ...
                write (newin,1090)
                write (newin,1220)
1220 format('| fixed fugacity phases- species, moles(per kg h2o), ','log fugacity(bars)',t72,'|')
...
1240 format('| none',t38,'|',t55,'|',t72,'|n')
                ...
                ! Reactants and rate laws.
                write (newin,1090)
                write (newin,1250)
1250 format('c',t20,'R A T E     L A W S',/,'c 1 = relative',t33,'rate = rk1 + rk2*zi + (1/2)rk3*zi*zi',/'c 2 = transition state theory',t33,'rate = CHECK DOCUMENTATION',/'c 3 = specified rate',/'c 4 = activity term rate',t33,'rate = CHECK DOCUMENTATION',/'c',/'c ',t20,'R E A C T A N T     T Y P E S',/'c mineral     solid solution     special    aqueous    gas',/'c',/'c ',t20,'S U R F A C E    T Y P E',/'c 0 = fixed surface area     1 = fixed specific surface area',/'c',/,'c',t20,'N O T E S',/'c status and jreac are normally not set by the user')

                write (newin,1090)
                write (newin,1260)
1260 format('| reactants   (ss) solid solution only',5x,'(sp) special reactant only  |')

                write (newin,1090)

                if (nrct .eq. 0) then
                    write (newin,1270)
1270 format('| REACTANT',t20,'|  none',t47,'|status',t57,'|',t72,'|')
                else
                    nsrt = 0
                    nxrt = 0
...
1290 format('| REACTANT',t20,'| ',a24,t47,'|status',t57,'|',i2,t72,'|')

                        write (newin,1300) morr(nrc),modr(nrc)
1300 format('| moles remaining',t20,'|',1pg12.5,t47,'|destroyed',t57,'|',1pg12.5,t72,'|')

                        write (newin,1310) umint(jcode(nrc)),sk(nrc)
1310 format('| reactant type',t20,'|',2x,a14,t47,'|sk',t57,'|',1pg12.5,t72,'|')

                        write (newin,1320) nsk(nrc),fk(nrc)
1320 format('| surface type',t20,'|',2x,i2,t47,'|fk',t57,'|',1pg12.5,t72,'|')
...
1330 format('| end-member',t20,'| ',a24,t47,'|mole fr',t57,'|',1pg12.5,t72,'|ss,n')
...
170 continue
                                ...
                                write (newin,1340)
1340 format('| end-member',t20,'|',t47,'|mole fr',t57,'|',t72,'|ss,n')
                            end if
...
1350 format('| volume',t20,'|',1pg12.5,t47,'|',t57,'|',t72,'|sp')

                                do 190 ncb = 1,nesrbt(nsrt)
                                    write (newin,1360) uesrb(ncb,nsrt),cesrb(ncb,nsrt)
1360 format('| element',t20,'| ',a8,t35,'|moles',t47,'|',1x,1pe22.15,t72,'|sp,n')
...
190 continue
                                    ...
                                    write (newin,1370)
1370 format('| volume',t20,'|',t47,'|',t57,'|',t72,'|sp',/'| element',t20,'|',t47,'|moles',t57,'|',t72,'|sp,n')
                                end if

                                ! Rate law parameters, forward direction.
                                write (newin,1380) nrk(1,nrc)
1380 format('| DISSOLUTION LAW  |',i5,t47,'|',t57,'|',t72,'|')

                                if (nrk(1,nrc) .eq. 1) then
                                    do 210 i = 1,imech(1,nrc)
                                        write (newin,1390) i,rk0(i,1,nrc),i
1390 format('| rate constant rk',i1,t20,'|',1pg12.5,t47,'|csigma',i1,t57,'|',t72,'|')

210 continue
                                    else if (nrk(1,nrc) .eq. 2) then
                                        do 230 i = 1,imech(1,nrc)
                                            write (newin,1395) i,rk0(i,1,nrc),i,csigma(i,1,nrc)
1395 format('| rate constant rk',i1,t20,'|',1pg12.5,t47,'|csigma',i1,t57,'|',1pg12.5,t72,'|')

                                            do 220 n = 1,ndact(i,1,nrc)
                                                write (newin,1400) udac(n,i,1,nrc),cdac(n,i,1,nrc)
1400 format('| aqueous species  | ',a24,t47,'|cdac',t57,'|',1pg14.5,t72,'|n')

220 continue

                                                write (newin,1410) trk0(i,1,nrc)
1410 format('| temperature (c)  | ',1pg12.5,t47,'|',t57,'|',t72,'|234')

                                                if (iact(i,1,nrc) .eq. 1) then
                                                    write (newin,1420) eact(i,1,nrc)
1420 format('| act. energy-kcal | ',1pg12.5,t47,'|enthalpy',t57,'|',t72,'|234')
                                                else if (iact(i,1,nrc) .eq. 2) then
                                                    write (newin,1430) hact(i,1,nrc)
1430 format('| act. energy-kcal | ',t47,'|enthalpy',t57,'| ',1pg12.5,t72,'|234')
                                                end if
...
1440 format('| PRECIPITATION LAW|',i5,t47,'|',t57,'|',t72,'|')
...
390 continue
                                                                                    ...
                                                                                    ! Options. Iopt, iopr, and iodb option switches, but not iopg
...
1465 format('| - ',a,' -',t72,'|')
                                                                                            ...
                                                                                            do 400 idesc = 1,nod6pa
                                                                                                if (iopti6(idesc) .eq. idescx) then
                                                                                                    ...
                                                                                                    else if (uvar6(idescx)(1:4) .eq. 'iopr') then
                                                                                                        if (iopr(index6(idescx)) .eq. ivalu6(idesc)) then
                                                                                                            ux1(1) = '*'
                                                                                                        else
                                                                                                            ux1(1) = ' '
                                                                                                        end if
                                                                                                    else if (uvar6(idescx)(1:4) .eq. 'iodb') then
...
410 continue
                                                                                            ...
                                                                                            write (newin,1090)
                                                                                            write (newin,1480)
1480 format('| development options  (used for code development)',t72,'|')
                                                                                            ...
                                                                                            do 420 idescx = 1,ndv6pa
                                                                                                ...
                                                                                                else if (udv6vr(idescx)(1:4) .eq. 'iopr') then
                                                                                                    ival = iopr(idev6i(idescx))
                                                                                                else if (udv6vr(idescx)(1:4) .eq. 'iodb') then
                                                                                                    ival = iodb(idev6i(idescx))
                                                                                                else if (udv6vr(idescx)(1:4) .eq. 'iopg') then
...
1530 format('| ',a32,'|',1pg12.5,t54,'|',2x,a14,t72,'|')
                                                                                                ...
                                                                                                ! Bottom half of the current output file.
                                                                                                ! Write recaptured data in comments on the originating code, if
                                                                                                ! known.
                                                                                                if (ucode(1:3).eq.'EQ6' .or. ucode(1:3).eq.'eq6') then
                                                                                                    write (newin,3000) urelno,ustage,ueqlrn,ueqlst
3000 format('c pickup file written by EQ6, version ',a4,' (',a8,')',t72,'|',/'c  supported by EQLIB, version ',a4,' (',a8,')',t72,'|')
                                                                                                end if

                                                                                                if (ucode(1:5).eq.'EQ3NR' .or. ucode(1:5).eq.'eq3nr') then
                                                                                                    write (newin,3005) urelno,ustage,ueqlrn,ueqlst
3005 format('c pickup file written by EQ3NR, version ',a4,' (',a8,')',t72,'|',/'c  supported by EQLIB, version ',a4,' (',a8,')',t72,'|')
                                                                                                end if
...
3040 format('| electrical imbalance',t38,'|',2x,1pe22.15,t72,'|')
...
3070 format('| position of last solid solution',t38,'|',2x,i5,t72,'|')
                                                                                                    ...
                                                                                                    ! Nxmod options.
                                                                                                    write (newin,3080)
3080 format('| suppressed species',5x,'(suppress,replace,augmentk,augmentg)  value',t72,'|')
...
3090 format('| ',a24,t28,'| ',a14,' | ',a8,' | ',1pg12.5,t72,'|')

520 continue
                                                                                                        else
                                                                                                            write(newin,3090) unone,ublk24,ublk8,zero
                                                                                                        end if
...
3140 format('| ',a8,t28,'| ',1pe22.15,' | ',1pe22.15,' |')
...
560 continue
                                                                                                                        ...
                                                                                                                        write (newin,3170)
3170 format('| physically removed subsystem ',' (solid solution, mineral, moles)',t72,'|')
...
3190 format('| ',a24,' |',t55,'|',t80,'|')

                                                                                                                                        uxx = uprs(npr)(1:24)
                                                                                                                                    end if

                                                                                                                                    write (newin,3200) uprs(npr)(25:48),mprs(npr)
3200 format('|',t28,'| ',a24,' | ',1pe22.15,' |')

590 continue
                                                                                                                                else
                                                                                                                                    write (newin,3190) unone
                                                                                                                                end if
