subroutine ssortw(asort,aval,jsort,nmax,noutpt,nttyo,nval)
    ...
    !! Recipes" book.
    !! The sorted elements of "aval" are placed in the array "asort,"
    ...
    !! should be a good starting point for the present sorting.
    !! All the arrays are dimensioned 1:nmax.
    !! EQLIBU/qsortw.f is a possible alternative to this subroutine. It
    !! uses the "quicksort" method.
    ...
    !!            composed of the first nval elements fo the aval array
...
1000 format(/' * Warning - (EQLIBU/ssortw) Programming error',' trap: The minimum',/7x,'element of the incoming jsort',' array is ',a,', not 1 as is required.',/7x,'Will ignore',' this jsort array and create a new one from scratch.')
...
1010 format(/' * Warning - (EQLIBU/ssortw) Programming error',' trap: The maximum',/7x,'element of the incoming jsort',' array is ',a,', not nval (',a,'),',/7x,'as is required.',' Will ignore this jsort array and create a new one',/7x,'from scratch.')
    ...
    do n1 = 1,ilognp
        ...
        do j = 1,k
            ...
            ! The label below is the return point for a loop which
            ! is run through until no out-of-order elements are found.
120 continue
    ...
    ! Load the asort array. Note that the loop is unrolled.
    do jpt = 1,ileft,8
        asort(jpt) = aval(jsort(jpt))
        asort(jpt + 1) = aval(jsort(jpt + 1))
        asort(jpt + 2) = aval(jsort(jpt + 2))
