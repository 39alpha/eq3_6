subroutine fmspnx(jlen,unam48,uspn56)
    ...
    !! 48-character string, if the phase name is identical to the
    !! species name, or if the phase name is 'Aqueous solution'.
    !! Thus, the returned string for pure Albite is 'Albite', not
    !! 'Albite (Albite)', and that for the aqueous sulfate ion is
    !! 'SO4--', not 'SO4-- (Aqueous solution)'.
    !! The formatted string is used mostly in error, warning, and note
    !! messages written by EQ3NR and EQ6.
    !! This subroutine is similar to fmspnm.f. However, that subroutine
    !! includes the phase part if the phase name is 'Aqueous solution'.
    ...
    if (uphnam(1:24) .ne. unam48(1:24)) then
        if (uphnam(1:24) .ne. 'Aqueous solution        ') then
            if (j2 .gt. 0) then
