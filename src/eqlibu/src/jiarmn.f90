integer function jiarmn(iarray,nmax)
    !! This subroutine finds the minimum element of the integer array
    ...
    !! it in the following manner: call jiarmn(ia,(i1*i2)). Use this
    !! subroutine with caution if nmax is not the product of the true
    !! (declared) dimensions.
    !! To find the corresponding maximum value, use EQLIBU/jiarmx.f
    !! instead.
    ...
    !! Output:
    !!   jiarmn = the minimum element of the array (zero if nmax .le. 0)
    implicit none
    ...
    do i = 1,ileft,8
        ix = min(ix,iarray(i),iarray(i + 1),iarray(i + 2),iarray(i + 3),iarray(i + 4),iarray(i + 5),iarray(i + 6),iarray(i + 7))
    end do
