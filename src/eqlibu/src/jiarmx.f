...
c
c     This subroutine finds the maximum element of the integer array
...
c     it in the following manner: call jiarmx(ia,(i1*i2)). Use this
c     subroutine with caution if nmax is not the product of the true
c     (declared) dimensions.
c
c     To find the corresponding minimum value, use EQLIBU/jiarmn.f
c     instead.
c
...
c     Output:
c
c       jiarmx = the maximum element of the array (zero if nmax .le. 0)
c
c-----------------------------------------------------------------------
c
      implicit none
...
c
      do i = 1,ileft,8
        ix = max(ix,iarray(i),iarray(i + 1),iarray(i + 2),iarray(i + 3),
