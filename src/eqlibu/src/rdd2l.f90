subroutine rdd2l(nfldmx,nfldt,nfldtx,ninpts,nlchmx,nttyo,qrderr,ufield,uline1,uline2,ulscr)
    ...
    !! EQLIBU/rdd2lh.f is very similar to the present subroutine. It
    !! differs in that the header is required to match a defined string.
...
1010 format(/' * Warning - (EQLIBU/rdd2l) Found ',i2,' fields',/7x,'where ',i2,' were expected on the line beginning with',/7x,'"',a,'".')
        end if
    end if
...
1030 format(/' * Error - (EQLIBU/rdd2l) Found the line beginning',' with',/7x,'"',a,'"',/7x,'where a dashed separator line was expected.')
