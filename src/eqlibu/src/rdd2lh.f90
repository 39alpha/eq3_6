subroutine rdd2lh(nfldmx,nfldt,nfldtx,ninpts,nlchmx,nttyo,qrderr,ufield,uheadr,uheadx,uline1,uline2,ulscr)
    !! This subroutine reads a "2 line" block containing an expected
    !! header or tag string from an EQ3/6 input in menu-style ("D")
    !! format. The first line contains the header and data. The second
    !! line is a separator line containing a single field of dashes.
    !! EQLIBU/rdd2l.f is very similar to this subroutine. It differs in
    !! that the header is not expected to be known in advance.
    !! This subroutine is called by:
    !!   XCON3/rd3d7.f
    !!   XCON6/rd6d7.f
    !! Input:
    !!   nfldmx = the maximum number of fields on line uline1 or
    !!              line uline2
    !!   nfldtx = the expected number of fields on a line
    !!   nlchmx = the maximum number of characters on a line
    !!   ninpts = the unit number of the stripped input file
    !!   nttyo  = the unit number of the screen file
    !!   uheadx = the expected header (the expected contents of the
    !!              first of the two lines)
    !!   ulscr  = scratch character variable
    !! Output:
    !!   nfldt  = the number of fields on line uline1 or line uline2
    !!   qrderr = logical flag, true if an error was encountered
    !!   ufield = the array of fields on a line
    !!   uheadr = the header (contents of the first of the two lines)
    !!   uline1 = the first line to be read
    !!   uline2 = the second line to be read
    implicit none

    ! Calling sequence variable declarations.
    integer :: nttyo

    integer :: nfldmx
    integer :: nfldt
    integer :: nfldtx
    integer :: ninpts
    integer :: nlchmx

    logical :: qrderr

    character(len=*) :: ufield(nfldmx)
    character(len=*) :: uline1
    character(len=*) :: uline2
    character(len=*) :: ulscr
    character(len=*) :: uheadr
    character(len=*) :: uheadx

    ! Local variable declarations.
    integer :: j
    integer :: j2
    integer :: j3

    integer :: ilnobl

    qrderr = .false.

    ! Read and parse the first line.
    read (ninpts,1000,err=990) uline1
1000 format(a80)

    call parslj(nfldmx,nfldt,nlchmx,ufield,uline1,ulscr)

    ! Compare the number of fields found with that expected.
    if (nfldtx .gt. 0) then
        if (nfldt .ne. nfldtx) then
            j3 = ilnobl(uline1(1:70))
            write (nttyo,1010) nfldt,nfldtx,uline1(1:j3)
1010 format(/' * Warning - (EQLIBU/rdd2lh) Found ',i2,' fields',/7x,'where ',i2,' were expected on the line beginning with',/7x,'"',a,'".')
        end if
    end if

    ! Compare the header with that expected.
    uheadr = ufield(1)
    call locase(uheadr)
    call locase(uheadx)

    j2 = ilnobl(uheadx)
    j = index(uheadr,uheadx(1:j2))

    if (j .eq. 0) then
        qrderr = .true.
        j3 = ilnobl(uline1(1:70))
        j2 = ilnobl(uheadx(1:70))
        write (nttyo,1020) uheadx(1:j2),uline1(1:j3)
1020 format(/' * Error - (EQLIBU/rdd1lh) Was expecting to find the',/7x,'header beginning with"',/7x,'"',a,'"',/7x,'on the line beginning with',/7x,'"',a,'".')

        go to 999
    end if

    ! Read the second line.
    read (ninpts,1000,err=990) uline2

    if (uline2(2:9) .eq. '--------') then
        go to 999
    end if

    qrderr = .true.
    j3 = ilnobl(uline2(1:70))
    write (nttyo,1030) uline2(1:j3)
1030 format(/' * Error - (EQLIBU/rdd2lh) Found the line beginning',' with',/7x,'"',a,'"',/7x,'where a dashed separator line was expected.')

    go to 999

990 continue
    qrderr = .true.

999 continue
end subroutine rdd2lh